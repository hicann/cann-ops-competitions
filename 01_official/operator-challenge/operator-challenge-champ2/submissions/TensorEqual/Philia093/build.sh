...
chmod +x "$script_path"/build.sh "$script_path"/scripts/*.sh "$script_path"/cmake/util/*.sh 2>/dev/null || true
...
export OPS_PRODUCT_NAME=$(python3 - <<'PY' "$script_path/CMakePresets.json"
import json
import sys

with open(sys.argv[1], "r", encoding="utf-8") as handle:
    data = json.load(handle)
cache = data["configurePresets"][0].get("cacheVariables", {})
entry = cache.get("ASCEND_COMPUTE_UNIT", "ascend910b")
print(entry.get("value", "ascend910b") if isinstance(entry, dict) else entry)
PY
)
echo "using OPS_PRODUCT_NAME: $OPS_PRODUCT_NAME"
...
ensure_autogen() {
  if [ -f "$BUILD_DIR/autogen/op_proto.cc" ]; then
    return
  fi
  if [ ! -f "$BUILD_DIR/autogen/libascend_all_ops.so" ]; then
    return
  fi
  echo "regenerating autogen sources for OPS_PRODUCT_NAME=$OPS_PRODUCT_NAME"
  "$ASCEND_HOME_PATH/toolkit/tools/opbuild/op_build" \
    "$BUILD_DIR/autogen/libascend_all_ops.so" \
    "$BUILD_DIR/autogen"
}
