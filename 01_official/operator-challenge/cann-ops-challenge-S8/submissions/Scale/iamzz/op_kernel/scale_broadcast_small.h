...
template <typename T>
class KernelScale_broadcast_small {
public:
    __aicore__ inline KernelScale_broadcast_small() {}
    ...
    __aicore__ inline void Process_iter(uint32_t offset, uint32_t iterSize) {
        ...
        if constexpr (std::is_same<T, bfloat16_t>::value) {
            BinaryBroadcastFirstAxis(scaleLocal.template ReinterpretCast<half>(), scaleLocal.template ReinterpretCast<half>(), n_lines_per_iter,
                                     n_elements_per_iter);
            if (hasBias) {
                BinaryBroadcastFirstAxis(biasLocal.template ReinterpretCast<half>(), biasLocal.template ReinterpretCast<half>(), n_lines_per_iter,
                                         n_elements_per_iter);
            }
        } else if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, uint8_t>::value || std::is_same<T, bool>::value) {
            BinaryBroadcastFirstAxis(scaleLocal.template ReinterpretCast<int16_t>(), scaleLocal.template ReinterpretCast<int16_t>(), n_lines_per_iter,
                                     n_elements_per_iter / 2);
            if (hasBias) {
                BinaryBroadcastFirstAxis(biasLocal.template ReinterpretCast<int16_t>(), biasLocal.template ReinterpretCast<int16_t>(), n_lines_per_iter,
                                         n_elements_per_iter / 2);
            }
        } else if constexpr (std::is_same<T, int64_t>::value) {
            return;
        } else {
            BinaryBroadcastFirstAxis(scaleLocal, scaleLocal, n_lines_per_iter, n_elements_per_iter);
            if (hasBias) {
                BinaryBroadcastFirstAxis(biasLocal, biasLocal, n_lines_per_iter, n_elements_per_iter);
            }
        }
