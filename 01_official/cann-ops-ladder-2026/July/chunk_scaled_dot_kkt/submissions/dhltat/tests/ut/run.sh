#!/bin/bash
# chunk_scaled_dot_kkt 算子 UT 测试执行脚本
...
echo "========================================"
echo "chunk_scaled_dot_kkt 算子 UT 测试"
echo "========================================"
...
if ./chunk_scaled_dot_kkt_op_host_ut; then
...
echo "========================================"
DATA_DIR="${SCRIPT_DIR}/op_kernel/chunk_scaled_dot_kkt_data"
cd "${DATA_DIR}"
python3 gen_data.py

# 运行 Kernel UT
cd "${BUILD_DIR}/op_kernel"
if ./chunk_scaled_dot_kkt_op_kernel_ut; then
...
if [ -d "${BUILD_DIR}/../../../op_api" ]; then
    cd "${BUILD_DIR}/op_api"
    if ./chunk_scaled_dot_kkt_op_api_ut; then
        PASSED_TESTS+=("op_api")
    else
        FAILED_TESTS+=("op_api")
    fi
