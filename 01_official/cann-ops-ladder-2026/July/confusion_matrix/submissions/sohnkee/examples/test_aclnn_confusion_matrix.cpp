...
#include <algorithm>
#include "acl/acl.h"
#include "aclnn_confusion_matrix.h"
...
int main()
{
    ...
    void* predictionsDeviceAddr = nullptr;
    aclTensor* weights = nullptr;
    void* weightsDeviceAddr = nullptr;
    std::vector<int64_t> labelsShape = {5};
    std::vector<int32_t> labelsHostData(5, 1);
    ret = CreateAclTensor(labelsHostData, labelsShape, &labelsDeviceAddr, aclDataType::ACL_INT32, &labels);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> predictionsShape = {5};
    std::vector<int32_t> predictionsHostData(5, 1);
    ret = CreateAclTensor(predictionsHostData, predictionsShape, &predictionsDeviceAddr, aclDataType::ACL_INT32, &predictions);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 构造输出 tensor
    aclTensor* y = nullptr;
    void* yDeviceAddr = nullptr;
    std::vector<int64_t> yShape = {3, 3};
    std::vector<int32_t> yHostData(9, 0);
    ret = CreateAclTensor(yHostData, yShape, &yDeviceAddr, aclDataType::ACL_INT32, &y);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    

    // 调用 aclnnConfusionMatrixGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnConfusionMatrixGetWorkspaceSize(labels, predictions, weights, 3, "int32", y, &workspaceSize, &executor);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnConfusionMatrixGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnConfusionMatrix 第二段接口
    ret = aclnnConfusionMatrix(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnConfusionMatrix failed. ERROR: %d\n", ret); return ret);
    ...
    aclrtFree(predictionsDeviceAddr);
