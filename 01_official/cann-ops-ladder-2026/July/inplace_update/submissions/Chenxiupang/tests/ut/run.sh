...
if ./inplace_update_op_host_ut; then
...
echo "========================================"
DATA_DIR="${SCRIPT_DIR}/op_kernel/inplace_update_data"
cd "${DATA_DIR}"
python3 gen_data.py

# 运行 Kernel UT
cd "${BUILD_DIR}/op_kernel"
if ./inplace_update_op_kernel_ut; then
...
if [ -d "${BUILD_DIR}/../../../op_api" ]; then
    cd "${BUILD_DIR}/op_api"
    if ./inplace_update_op_api_ut; then
        PASSED_TESTS+=("op_api")
    else
        FAILED_TESTS+=("op_api")
    fi
