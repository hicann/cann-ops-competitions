...
#include <algorithm>
#include "acl/acl.h"
#include "aclnn_inplace_update.h"
...
int main()
{
    ...
    std::vector<int64_t> xShape = {1024, 1280};
    std::vector<float> xHostData(1310720, 1);
    ret = CreateAclTensor(xHostData, xShape, &xDeviceAddr, aclDataType::ACL_FLOAT, &x);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> iShape = {1};
    std::vector<int32_t> iHostData(1, 1);
    ret = CreateAclTensor(iHostData, iShape, &iDeviceAddr, aclDataType::ACL_INT32, &i);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    std::vector<int64_t> vShape = {1, 1280};
    std::vector<float> vHostData(1280, 1);
    ...
    std::vector<int64_t> yShape = {1024, 1280};
    std::vector<float> yHostData(1310720, 0);
    ret = CreateAclTensor(yHostData, yShape, &yDeviceAddr, aclDataType::ACL_FLOAT, &y);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    

    // 调用 aclnnInplaceUpdateGetWorkspaceSize 第一段接口
    uint64_t workspaceSize = 0;
    aclOpExecutor* executor = nullptr;
    ret = aclnnInplaceUpdateGetWorkspaceSize(x, i, v, y, &workspaceSize, &executor);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnInplaceUpdateGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
    ...
    // 调用 aclnnInplaceUpdate 第二段接口
    ret = aclnnInplaceUpdate(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACLNN_SUCCESS, LOG_PRINT("aclnnInplaceUpdate failed. ERROR: %d\n", ret); return ret);
