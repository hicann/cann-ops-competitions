...
const std::string Atan2Test::rootPath = "../../../../experimental/";
const std::string Atan2Test::dataPath = rootPath + "math/atan2/tests/ut/op_kernel/atan2_data";
...
TEST_F(Atan2Test, test_case_float32_1)
{
    uint32_t blockDim = 1;
    system("cd ./atan2_data/ && python3 gen_data.py '(4, 2)' 'float32'");
    uint32_t dataCount = 8;
    size_t inputByteSize = dataCount * sizeof(float);
    std::string x1_fileName = "./atan2_data/float32_input1_t_atan2.bin";
    std::string x2_fileName = "./atan2_data/float32_input2_t_atan2.bin";
    ...
    auto func = atan2<ELEMENTWISE_TPL_SCH_MODE_1>;
    ICPU_RUN_KF(func, blockDim, x1, x2, y, workspace, (uint8_t*)(tilingData));

    std::string fileName = "./atan2_data/float32_output_t_atan2.bin";
    ...
    system("cd ./atan2_data/ && python3 compare_data.py 'float32'");
}   