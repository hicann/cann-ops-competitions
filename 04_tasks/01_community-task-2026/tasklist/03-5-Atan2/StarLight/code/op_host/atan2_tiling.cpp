...
namespace optiling {
    ...
    const uint32_t DATA_NUM_32B = 10;
    const uint32_t DATA_NUM_16B = 23;
                                     ...
                                     uint64_t& inputLengthAlgin, uint32_t& bufferNum)
    {
        ...
        {
            case ge::DT_FLOAT:
                ubDataNumber = DATA_NUM_32B;  // 输入1 + 输入2 + 输出
                break;
            case ge::DT_BF16:
            case ge::DT_FLOAT16:
                ubDataNumber = DATA_NUM_16B;  // 可能需要更多中间缓冲区
                break;
            default:
                break;
        }
    ...
    static ge::graphStatus Atan2TilingFunc(gert::TilingContext* context)
    {
        ...
        ret = CalculateCoreBlockNums(inputLengthAlgin, coreNum, tileBlockNum, inputBytes,
