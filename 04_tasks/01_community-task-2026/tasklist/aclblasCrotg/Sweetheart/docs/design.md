...
6. 标量热路径开销控制：rtmax 阈值 constexpr 预计算（host/kernel 各 2 处，消除运行时 sqrtf）、去掉每次调用的 OP_LOGD/OP_LOGI（仅保留错误路径 OP_LOGE）、tiling 不做冗余零初始化、精简 kernel 启动函数签名。
...
总体沿用 srotg arch22 的"极简标量算子"范式：**无 workspace、无 UB 张量缓冲、无 DataCopy 流水、无多核切分、无 tilingKey**；device 路径仅启动 1 个 AIV block，核内通过 `GlobalTensor::GetValue/SetValue` 直接访问 4 个标量 GM 地址，计算量仅数十条标量浮点指令（2~4 次 sqrt），耗时完全由 kernel launch 固定开销主导。host 与 kernel 各保留一份**同构算法副本**（同常量、同分支顺序、同运算次序），避免 device 编译器引入 host 头文件。
...
**（5）标量热路径开销控制（code-915 相对早期归档的优化）**

1. 删除每次调用的 `OP_LOGD("tiling...")`/`OP_LOGI("launching kernel")`，仅保留错误路径 `OP_LOGE`：默认日志级别下省两次日志门检；INFO/DEBUG 排障级别下避免每调用 2~4 μs 的真实格式化打印；
2. 两个 rtmax 阈值改 constexpr 预计算（host + kernel 同步），消除每次进入缩放分支的 sqrtf，且与运行时 sqrtf 位级一致；
3. 4 次指针属性查询复用同一个 `aclrtPtrAttributes`（调用方零初始化一次），避免每次查询重填约 32 字节栈；
4. tiling 4 字段全赋值，去掉 `= {}` 冗余零填充；
5. `crotg_kernel_do` 只保留 `(tiling, stream)` 两个参数（4 个操作数指针与 numBlocks=1 均为冗余入参，地址全部由 tiling 承载）。
