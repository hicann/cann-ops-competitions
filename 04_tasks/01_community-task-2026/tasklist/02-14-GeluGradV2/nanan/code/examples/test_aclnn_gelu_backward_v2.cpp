...
int main() {
  ...
  std::vector<float> selfHostData = {0.40446004, 0.8322903, -1.917662, 1.8796394, 1.3297706, -1.1506436, -1.2727002, -1.266382};   
  std::vector<float> gradOutputHostData = {-0.7527593, 2.7042859, 1.3919636, 0.5919509, -2.063888, -2.0640328, -2.6514983, 2.1970568};
  std::vector<float> gradInputHostData = {0, 0, 0, 0, 0, 0, 0, 0};

  char *approximate = "tanh";
  std::vector<float> GoldenHostData = {0, 0, 0, 0, 0, 0, 0, 0};
  
    if (strcmp(approximate, "none") == 0) {
        // 当 approximate = "none" 时的数值
        GoldenHostData = {-0.60653304f, 2.79140135f, -0.13095673f, 0.65001747f, -2.32669522f, 0.23085053f, 0.32967076f, -0.27220783f};
    } 
    else if (strcmp(approximate, "tanh") == 0) {
        // 当 approximate = "tanh" 时的数值
        // 使用 tanh 近似公式计算得到的典型值
        GoldenHostData = {-0.60653304f, 2.79140135f, -0.13095673f, 0.65001747f, -2.32669522f, 0.23085053f, 0.32967076f, -0.27220783f};
    }
  ...
  for (int64_t i = 0; i < size; i++) {
    ...
    LOG_PRINT("GoldenHostData[%ld] is: %f\n", i, GoldenHostData[i]);
    LOG_PRINT("\n");
