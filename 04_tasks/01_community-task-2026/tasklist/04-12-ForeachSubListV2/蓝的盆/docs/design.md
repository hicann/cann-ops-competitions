# aclnnForeachSubListV2 算子设计文档

## 一、 需求背景

### 1.1 需求来源

通过社区任务完成开源仓算子贡献的需求，补充完善 Ascend C 算子库。为现有的 `aclnnForeachSubListV2` 算子增加对 `DT_INT16`、`DT_INT8`、`DT_UINT8` 数据类型的支持，以满足更广泛的网络量化及端侧推理场景对底层核心算子的完备性要求。

### 1.2 背景介绍

`ForeachSubListV2` 算子用于对两个张量列表（Tensor List）执行逐元素的带缩放减法运算。其核心数学计算公式为：
$y_i = x1_i - \alpha \times x2_i$
根据最新算子原型定义，当前算子已支持 `FLOAT16`, `FLOAT32`, `INT32`, `BFLOAT16` 数据类型。本项目要求在现有代码架构下，新增对 `INT16`、`INT8`、`UINT8` 数据的支持。

对于新增的低精度整型数据，采用精度提升策略：`int16_t` 统一转换（Cast）为 `float`（float32）进行计算，`int8_t` 和 `uint8_t` 则转换至 `half` (float16) 进行计算，计算完成后再转换回原类型。该策略与已有 `BFLOAT16` 的 `Cast` 处理模式保持一致，可复用现有的 `InnerComputer` 模板特化框架。
...
| 模块 | 变更项 | 说明 |
| --- | --- | --- |
| `op_host` | `foreach_sub_list_def.cpp` | 扩展主配置的 `tensor_dtype_list`，新增 `DT_INT16/DT_INT8/DT_UINT8`。Kirin 芯片保持原有数据类型不变。 |
| `op_host` | `foreach_tiling_func.cpp` | **修改 UB 切分策略**：`int16_t` 按照中间缓存 `float32` (4 Bytes) 预留内存；`int8_t/uint8_t` 按照 `half` (2 Bytes) 预留内存。 |
| `op_kernel` | `foreach_sub_list.cpp` | 新增 `TILING_KEY_IS` 路由分支：`int16_t` 路由至 `float` 适配器；`int8/uint8` 路由至 `half` 适配器。 |
| `op_kernel` | `foreach_one_scalar_ternary.h` | 扩展 `InnerComputer` 模板特化，新增 `int16_t -> float` 及 `8-bit -> half` 执行运算的双向转换流水线编排。 |
| `op_api` | `aclnn_foreach_sub_list_v2.cpp` | 从调用层面上增加三种新增数据类型的支持。 |
...
修改 `op_host/foreach_sub_list_def.cpp`，同步更新数据类型：


**硬件配置说明**：
- `ascend910_93`、`ascend910b`：支持全部 7 种数据类型（含新增的 INT16/INT8/UINT8）
- 其他硬件：保持原有 3 种数据类型（FLOAT16/FLOAT/INT32），不扩展新类型
...
在 `foreach_sub_list.cpp` 中，增加针对三种新类型的入口调用，统一利用 `SubListFloatAdapter` 的高精度指令（Axpy）：
...
// ... 现有 FLOAT16, FLOAT32, INT32, BFLOAT16 分支 ...
        ...
        // int8_t 转 half 计算
        ForeachOneScalarTernary<int8_t, half, SubListFloatAdapter<half>> op;
        op.Init(inputs_1, inputs_2, alpha, outputs, userWS, &tilingData);
        op.Process();
    } else if (TILING_KEY_IS(8)) {
        // uint8_t 转 half 计算
        ForeachOneScalarTernary<uint8_t, half, SubListFloatAdapter<half>> op;
        op.Init(inputs_1, inputs_2, alpha, outputs, userWS, &tilingData);
        op.Process();
    }
#endif
...
![asce.png](https://raw.gitcode.com/user-images/assets/9516645/b619f2b0-d450-4892-99b3-a8eee42b59d3/asce.png 'asce.png')
...
| 文件路径 | 变更类型 | 变更说明 |
| --- | --- | --- |
| `op_host/foreach_sub_list_def.cpp` | 修改 | 原型扩展新增 `INT16/INT8/UINT8`； |
| `op_host/config/.../foreach_sub_list_binary.json` | 修改 | 补充新 dtype 的编译构建条目。 |
| `op_host/op_api/aclnn_foreach_sub_list_v2.cpp` | 修改 | 扩展新增 `INT16/INT8/UINT8`，适配外部调用。 |
| `op_kernel/foreach_sub_list.cpp` | 修改 | 引入针对 `TILING_KEY_IS(5)`, `(7)`, `(8)` 的识别。5 路由至 float 适配器，7 和 8 路由至 half 适配器。 |
