...
本需求来自 CANN 2026 社区算子任务。在昇腾 ops-blas 开源仓中补齐 aclblasSrotmg 面向 Atlas A2/A3 产品的 arch22 实现，使公共 BLAS 接口在 A2/A3 上具备与 Netlib SROTMG 和 cuBLAS cublasSrotmg 一致的功能、参数语义和异常行为。

### 1.2 背景介绍

SROTMG 用四个单精度标量 d1、d2、x1、y1 构造 modified Givens 旋转参数。输出通常直接传给 ROTM，在 QR 分解、最小二乘和迭代线性代数流程中用于消去二维向量的第二个分量。

ops-blas 已存在 Ascend 950PR/950DT 对应的 arch35 实现和公共 aclblasSrotmg API，但任务基线缺少 Atlas A2/A3 的 arch22 Host 与 Kernel 代码。A2/A3 调用方因此无法在保持同一 API 的前提下使用该算子。

### 1.3 aclblasSrotmg 算子实现与优化

本任务以现有公共接口和 arch35 代码组织为基础，使用 Ascend C 完成 arch22 适配，并针对纯标量控制流算子优化固定调用开销：
...
5. 缩放保护循环内增加 `isfinite` 检查，防止 Inf/NaN 输入导致无限循环。
...
| 本任务 arch22 Tiling 定义 | blas/rotmg/arch22/srotmg_tiling_data.h |
...
算子功能：根据 d1、d2、x1、y1 构造 modified Givens 变换 H，原地更新 d1、d2、x1，并输出 param[5]。
...
使用 Ascend C 为 aclblasSrotmg 实现 Atlas A2/A3 对应的 arch22 Device Kernel，并实现与 ops-blas 既有模式一致的 Host 入口。功能、flag 编码、缩放保护、原地更新、错误码和异步 stream 语义必须与任务书一致；有限输入以 cblas_srotmg 为 golden。
...
9. 在 Atlas 910B3 / CANN 9.1.0 上满足精度与性能门槛。
10. 提供完整 CSV、GTest、cblas golden 和可复现说明。
...
3. abs(q1) > abs(q2)：
   计算 h21=-y1/x1、h12=p2/p1、u=1-h12*h21。u>0 时 flag=0 并更新 d1、d2、x1；否则进入 flag=-1 全零分支。
4. abs(q1) <= abs(q2)：
   q2<0 时进入 flag=-1 全零分支；否则 flag=1，计算 h11=p1/p2、h22=x1/y1 和 u=1+h11*h22，再缩放并交换 d1、d2。
...
d1 或 abs(d2) 的非零有限值越出保护区间时，以 GAM^2 循环缩放，并同步补偿 x1 和 H 元素。首次缩放将 flag 展开为 -1。Host 与 Device 均设置 `std::isfinite` 检查；非有限值跳过缩放循环，避免正 Inf 触发无界 while。
...
本算子为纯标量算子：d1、d2、x1、y1 各 1 个 float，param 为 5 个 float。不存在维度、动态 shape、非连续 Tensor、broadcast、步长、前导维或空 Tensor。
...
```
aclblasSrotmg
-> handle/空指针校验 (ValidateSrotmgParams)
-> 查询五个数据指针位置 (SrotmgCheckPtrLocation)
-> 全 Host：SrotmgCpuCompute
-> 全 Device：LaunchSrotmgKernel -> srotmg_kernel<<<1, stream>>>
...
**文件**：`srotmg_host.cpp`（284行）

参数校验（`ValidateSrotmgParams`）：

1. handle==nullptr 返回 ACLBLAS_STATUS_HANDLE_IS_NULLPTR。
2. 任一数据指针为空返回 ACLBLAS_STATUS_INVALID_VALUE。
3. 检查当前 runtime context 是否已初始化，未初始化返回 ACLBLAS_STATUS_NOT_INITIALIZED。

指针位置检测（`SrotmgCheckPtrLocation`）：

使用 `aclrtPointerGetAttributes` 查询 d1、d2、x1、y1、param 的内存位置。全 Host 进入 CPU 计算，全 Device 发射 Kernel，混合位置返回 INVALID_VALUE。

Host 计算（`SrotmgCpuCompute`）：

全 Host 输入直接在 CPU 上完成标量运算，不创建 Device 缓冲区，不发射 Kernel，也不进行 H2D/D2H 搬运。算法逻辑与 Kernel 完全一致，包含完整的 Netlib srotmg 分支 + GAM 缩放保护 + isfinite 安全检查。

Tiling 策略：

本算子总数据量固定为 9 个 float（4 输入 + 5 输出），控制流串行且没有 shape。Host 侧不生成 tiling data，不申请 workspace，也不计算 tile 大小。Tiling 结构体仅用于传递 GM 地址给 Kernel。

分核策略：

Device 路径固定发射 1 个 AIV block。多核无法并行化单组标量依赖，且多个 block 会竞争写同一输出，因此不采用满核、大小核或尾块分配策略。

数据分块和内存策略：

不进行数据分块。四个输入标量和 param[5] 直接映射至 GM；有效载荷为 36 字节。Kernel 不创建 TPipe、TQue 或 UB 临时缓冲。
...
Device 路径使用 handle->stream 发射 Kernel。接口不主动同步，调用方在读回结果前同步同一 stream。

#### 3.2.3 Kernel 侧设计

**文件**：`srotmg_kernel.cpp`（210行）

Kernel 类型为 `KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY)`，固定 1 个 block。数据映射：
...
4. 对有限 d1、d2 执行缩放保护（isfinite 守卫）。
5. 按 flag 写入有效 param 槽（`StoreResults`）。
6. 写回 d1、d2、x1；不修改 y1。

Kernel 不划分 CopyIn、Compute、CopyOut 流水，因为 9 个标量不足以抵消 UB 搬运和队列建立的固定开销。直接 GM 标量访问更符合本算子的性能特征。
...
| Runtime context 未初始化 | NOT_INITIALIZED |
...
├── README.md
└── arch22/
    ├── srotmg_tiling_data.h      # Tiling 数据结构（21行）
    ├── srotmg_host.cpp           # Host 侧实现（284行）
    └── srotmg_kernel.cpp         # Device kernel 实现（210行）

test/rotmg/srotmg/
├── srotmg_golden.h               # cblas_srotmg golden 包装
├── srotmg_param.h                # 测试参数结构体
└── arch22/
    ├── srotmg_npu_wrapper.h      # NPU 封装（DeviceBuffer H2D/D2H）
    ├── srotmg_test.cpp           # GTest 测试代码（155行）
    ├── srotmg_test.csv           # 1200 条 CSV 驱动用例
    ├── gen_csv.py                # CSV 用例生成脚本
...
| 支持的芯片版本 | 支持情况 | 验证说明 |
|---|---|---|
| Atlas 800I/T A2（910B3） | 支持 | CANN 9.1.0 实机构建、功能、精度 PASS |
| Atlas A3 | 支持 | arch22 交叉构建 PASS |
...
4. 无 tiling 结构和 workspace 协议，减少跨版本维护面。

### 6.2 可测试性

1. 有限输入使用 Netlib BLAS cblas_srotmg 单一 golden。
2. 同一用例同时验证 Host（CsvDrivenHost）和 Device（CsvDrivenDevice）路径。
3. param[0] 精确比较，其余标量按 FLOAT32 容差逐项比较。
4. 覆盖空 handle（NullHandle TEST_F）、五种空数据指针、五种混合内存位置。
5. 覆盖均匀/正态随机分布、NaN/Inf、边界与多轮缩放。
6. Inf/NaN 用例通过 GTEST_SKIP() 跳过（cblas_srotmg golden 在这些输入上无限循环），CSV 保持原样不修改。
7. 空指针用例（TC_ED）通过 GTEST_SKIP() 跳过（CSV 无法表达真正 nullptr），由 NullHandle 单独覆盖。

### 6.3 精度、性能和内存标准

| 验收标准 | 描述 | 标准来源 |
|---|---|---|
| 精度标准 | FLOAT32：atol=2^-16≈1.53e-5，rtol=2^-10≈9.77e-4，flag 精确匹配 | CANN 生态算子精度标准、任务书 |
| 功能标准 | 1200 条 CSV + 1 条固定 GTest 全部通过，无失败 | 任务书和 ops-blas 测试框架 |
| 性能标准 | 纯标量算子，单次调用微秒级延迟；批量连续调用摊薄开销 | 任务书 |
...
| 分类 | 数量 | 覆盖 |
|---|---|---:|
| TC_L0 | 12 | flag=-2/-1/0/1 全路径基础功能 |
| TC_BR | 14 | 零、负零、相等与符号边界 |
| TC_SC | 18 | GAMSQ/RGAMSQ 和多轮缩放保护 |
| TC_FL | 9 | NaN、正负 Inf（GTEST_SKIP） |
| TC_ED | 6 | 五种空指针和全零合法输入（GTEST_SKIP） |
| TC_EX | 941 | 均匀分布 471、对数正态分布 470 |
| TC_PF | 200 | 三条硬门槛和吞吐扫描 |
| 固定 GTest | 1 | NullHandle 空句柄测试 |
...
4. 构建兼容：仅在 NPU_ARCH=dav-2201 时编译 arch22 代码。
