    ...
    if (totalNum > 0) {
        ...
        if (tilingData->dimNum > 1 && tilingData->activeDimCount > 0) {
            ...
            if (lastActiveDim == tilingData->dimNum - 1) {
                ...
                if (xDesc->GetDataType() == ge::DT_BF16 &&
                    tilingData->activeDimCount > 1 &&
                    tilingData->shapes[lastActiveDim] == 3 &&
                    totalBytes >= 512 &&
                    totalBytes <= 4096) {
                ...
                if (xDesc->GetDataType() == ge::DT_UINT8 && tilingData->shapes[lastActiveDim] <= 64) {
                    alignElements = std::max<int64_t>(alignElements, Lcm(elementsPerBlock, tilingData->shapes[lastActiveDim]));
                    if (tilingData->activeDimCount > 1 && tilingData->dimNum >= 6) {
                        ...
                        for (int64_t dim = 0; dim < lastActiveDim; ++dim) {
                            ...
                            minActiveStride =
                                minActiveStride == 0 ? tilingData->strides[dim] :
                ...
                const bool splitHugeTwoWayRoll = false;
                ...
                const bool splitFp16MultiLargeInnerRoll = xDesc->GetDataType() == ge::DT_FLOAT16 &&
                    ...
                    tilingData->strides[lastActiveDim] >= elementsPerBandwidthBlock &&
        ...
        const bool skipSingleCoreForTinyBf16Last =
            ...
            tilingData->shapes[lastActiveDimForTiny] == 3 &&
            totalBytes >= 512 &&
            totalBytes <= 4096;
