...
private:
    ...
    __aicore__ inline bool CopyStridedSingleElementByRowGather(int64_t dstIndex, int64_t srcBlockBase,
        int64_t sourceOffset, int64_t blockSize, int64_t blockCount, bool preferRowGatherPatch);
    __aicore__ inline bool CopyBlockRollByFlatPatch(
    ...
    __aicore__ inline void CopyMultiDimLastDimFullRowsFromSource(
        int64_t dstIndex, int64_t srcRowIndex, int64_t rowCount);
    __aicore__ inline void CopyMultiDimLastDimFullRowsBySegments(
        int64_t dstIndex, int64_t srcRowIndex, int64_t rowCount);
    ...
    bool preferRowGatherPatch)
{
    ...
    if (blockSize >= 15 && blockSize <= 31) {
        const int64_t rowGatherCap = tilingData_->dimNum == 2 && blockSize == 31 ? 256 :
            (blockSize == 31 ? (tilingData_->activeDimCount > 1 ? 384 : 384) :
                (tilingData_->dimNum == 2 ? 512 : 1024));
    ...
    while (copiedRows < blockCount) {
        ...
        if (tilingData_->useSafeUbShuffle != 0 || useRankGt2NarrowLayout) {
            const int64_t patchStride = useRankGt2NarrowLayout ? ROLL_GM_BLOCK_BYTES : 1;
            int64_t row = 0;
            const int64_t multiActiveRawInputBytes =
                blockSize == 15 ? 4 * 1024 * 1024 : 2 * 1024 * 1024;
            const bool isRank3MultiLastDimNarrow =
                tilingData_->dimNum == 3 &&
                tilingData_->activeDimCount > 1 &&
                tilingData_->shifts[tilingData_->dimNum - 1] != 0 &&
                tilingData_->shapes[tilingData_->dimNum - 1] == blockSize;
            const bool useTotalSizedRawInput =
                (blockSize == 15 || blockSize == 31 ||
                    (blockSize == 23 && tilingData_->totalNum * static_cast<int64_t>(sizeof(T)) >= 20 * 1024 * 1024)) &&
                isRank3MultiLastDimNarrow &&
                tilingData_->totalNum * static_cast<int64_t>(sizeof(T)) >= multiActiveRawInputBytes;
            const bool useMultiActiveRawInput =
                useRankGt2NarrowLayout && alignedBlockElements == ROLL_GM_BLOCK_BYTES &&
                tilingData_->activeDimCount > 1 &&
                (blockSize == 15 || blockSize == 23 || blockSize == 31) &&
                useTotalSizedRawInput;
            const bool useTargetNarrowUnroll =
                useRankGt2NarrowLayout && alignedBlockElements == ROLL_GM_BLOCK_BYTES &&
                ((tilingData_->activeDimCount == 1 && (blockSize == 7 || blockSize == 15 || blockSize == 31)) ||
                    useMultiActiveRawInput);
            if (useTargetNarrowUnroll) {
                const bool useRawInput = blockSize == 15 || blockSize == 23 || blockSize == 31;
                const bool useWideUnroll = blockSize == 7 ||
                    (blockSize == 15 && tilingData_->totalNum * static_cast<int64_t>(sizeof(T)) >= 4 * 1024 * 1024);
                if (useRawInput) {
                    auto inPtr = (__ubuf__ T*)inLocal.GetPhyAddr();
                    if (useWideUnroll) {
                        for (; row + 15 < currentRows; row += 16) {
                            const int64_t outBase = row * patchStride;
                            const int64_t inBase = row * alignedBlockElements + sourceOffset;
                            outLocal.SetValue(outBase, inPtr[inBase]);
                            outLocal.SetValue(outBase + patchStride, inPtr[inBase + alignedBlockElements]);
                            outLocal.SetValue(outBase + patchStride * 2, inPtr[inBase + alignedBlockElements * 2]);
                            outLocal.SetValue(outBase + patchStride * 3, inPtr[inBase + alignedBlockElements * 3]);
                            outLocal.SetValue(outBase + patchStride * 4, inPtr[inBase + alignedBlockElements * 4]);
                            outLocal.SetValue(outBase + patchStride * 5, inPtr[inBase + alignedBlockElements * 5]);
                            outLocal.SetValue(outBase + patchStride * 6, inPtr[inBase + alignedBlockElements * 6]);
                            outLocal.SetValue(outBase + patchStride * 7, inPtr[inBase + alignedBlockElements * 7]);
                            outLocal.SetValue(outBase + patchStride * 8, inPtr[inBase + alignedBlockElements * 8]);
                            outLocal.SetValue(outBase + patchStride * 9, inPtr[inBase + alignedBlockElements * 9]);
                            outLocal.SetValue(outBase + patchStride * 10, inPtr[inBase + alignedBlockElements * 10]);
                            outLocal.SetValue(outBase + patchStride * 11, inPtr[inBase + alignedBlockElements * 11]);
                            outLocal.SetValue(outBase + patchStride * 12, inPtr[inBase + alignedBlockElements * 12]);
                            outLocal.SetValue(outBase + patchStride * 13, inPtr[inBase + alignedBlockElements * 13]);
                            outLocal.SetValue(outBase + patchStride * 14, inPtr[inBase + alignedBlockElements * 14]);
                            outLocal.SetValue(outBase + patchStride * 15, inPtr[inBase + alignedBlockElements * 15]);
                        }
                    }
                    for (; row + 7 < currentRows; row += 8) {
                        const int64_t outBase = row * patchStride;
                        const int64_t inBase = row * alignedBlockElements + sourceOffset;
                        outLocal.SetValue(outBase, inPtr[inBase]);
                        outLocal.SetValue(outBase + patchStride, inPtr[inBase + alignedBlockElements]);
                    ...
                    if (useWideUnroll) {
                        for (; row + 15 < currentRows; row += 16) {
                            const int64_t outBase = row * patchStride;
                            const int64_t inBase = row * alignedBlockElements + sourceOffset;
                            outLocal.SetValue(outBase, inLocal.GetValue(inBase));
                            outLocal.SetValue(outBase + patchStride, inLocal.GetValue(inBase + alignedBlockElements));
                            outLocal.SetValue(outBase + patchStride * 2, inLocal.GetValue(inBase + alignedBlockElements * 2));
                            outLocal.SetValue(outBase + patchStride * 3, inLocal.GetValue(inBase + alignedBlockElements * 3));
                            outLocal.SetValue(outBase + patchStride * 4, inLocal.GetValue(inBase + alignedBlockElements * 4));
                            outLocal.SetValue(outBase + patchStride * 5, inLocal.GetValue(inBase + alignedBlockElements * 5));
                            outLocal.SetValue(outBase + patchStride * 6, inLocal.GetValue(inBase + alignedBlockElements * 6));
                            outLocal.SetValue(outBase + patchStride * 7, inLocal.GetValue(inBase + alignedBlockElements * 7));
                            outLocal.SetValue(outBase + patchStride * 8, inLocal.GetValue(inBase + alignedBlockElements * 8));
                            outLocal.SetValue(outBase + patchStride * 9, inLocal.GetValue(inBase + alignedBlockElements * 9));
                            outLocal.SetValue(outBase + patchStride * 10, inLocal.GetValue(inBase + alignedBlockElements * 10));
                            outLocal.SetValue(outBase + patchStride * 11, inLocal.GetValue(inBase + alignedBlockElements * 11));
                            outLocal.SetValue(outBase + patchStride * 12, inLocal.GetValue(inBase + alignedBlockElements * 12));
                            outLocal.SetValue(outBase + patchStride * 13, inLocal.GetValue(inBase + alignedBlockElements * 13));
                            outLocal.SetValue(outBase + patchStride * 14, inLocal.GetValue(inBase + alignedBlockElements * 14));
                            outLocal.SetValue(outBase + patchStride * 15, inLocal.GetValue(inBase + alignedBlockElements * 15));
                        }
                    }
                    for (; row + 7 < currentRows; row += 8) {
                        const int64_t outBase = row * patchStride;
            ...
            for (; row < currentRows; ++row) {
                outLocal.SetValue(row * patchStride, inLocal.GetValue(row * alignedBlockElements + sourceOffset));
            }
        } else {
            auto inPtr = (__ubuf__ T*)inLocal.GetPhyAddr();
            auto outPtr = (__ubuf__ T*)outLocal.GetPhyAddr();
            int64_t row = 0;
            if (alignedBlockElements == ROLL_GM_BLOCK_BYTES) {
                const int64_t source = sourceOffset;
                for (; row + 7 < currentRows; row += 8) {
                    const int64_t base = row * ROLL_GM_BLOCK_BYTES + source;
                    outPtr[row] = inPtr[base];
                    outPtr[row + 1] = inPtr[base + ROLL_GM_BLOCK_BYTES];
                    outPtr[row + 2] = inPtr[base + ROLL_GM_BLOCK_BYTES * 2];
                    outPtr[row + 3] = inPtr[base + ROLL_GM_BLOCK_BYTES * 3];
                    outPtr[row + 4] = inPtr[base + ROLL_GM_BLOCK_BYTES * 4];
                    outPtr[row + 5] = inPtr[base + ROLL_GM_BLOCK_BYTES * 5];
                    outPtr[row + 6] = inPtr[base + ROLL_GM_BLOCK_BYTES * 6];
                    outPtr[row + 7] = inPtr[base + ROLL_GM_BLOCK_BYTES * 7];
                }
            }
            for (; row < currentRows; ++row) {
        ...
        DataCopyExtParams copyOutParams;
        copyOutParams.blockCount = static_cast<uint16_t>(currentRows);
        copyOutParams.blockLen = 1;
        copyOutParams.srcStride = 0;
        copyOutParams.dstStride = static_cast<uint32_t>(blockSize - 1);
        DataCopyPad(yGm_[dstIndex + copiedRows * blockSize], result, copyOutParams);
...
__aicore__ inline bool Roll<T>::CopyBlockRollByFlatPatch(
    ...
    int64_t blockCount)
{
    if (blockCount <= 1 || firstElements <= 0 || secondElements <= 0 || blockSize <= 0) {
        return false;
    }
    ...
    const int64_t typeBytes = static_cast<int64_t>(sizeof(T));
    const int64_t smallElements = firstElements < secondElements ? firstElements : secondElements;
    const int64_t totalElements = blockCount * blockSize;
    const int64_t blockBytes = blockSize * typeBytes;
    const int64_t totalBytes = totalElements * typeBytes;
    const bool preferFp16P27Pre13LowPatch = false;
    const bool preferFp16MultiLastWidePatch = false;
    const bool preferRowGatherPatch = false;
    const bool preferBf16W3SmallPatch =
        IsSameType<T, bfloat16_t>::value &&
        tilingData_->activeDimCount > 1 &&
        tilingData_->dimNum > 1 &&
        tilingData_->shifts[tilingData_->dimNum - 1] != 0 &&
        tilingData_->shapes[tilingData_->dimNum - 1] == 3 &&
        blockSize == 3 &&
        smallElements == 1 &&
        totalBytes >= 512 &&
        totalBytes <= 4096;
    const bool preferBf16Rank3W19LargePatch = false;
    const bool allowWideShiftOnePatch = smallElements == 1 && blockSize <= ubElements_ &&
        blockBytes <= 8192 &&
        (totalBytes >= ROLL_FLAT_PATCH_MIN_TOTAL_BYTES || preferBf16W3SmallPatch);
    if ((!preferFp16P27Pre13LowPatch && !preferFp16MultiLastWidePatch &&
            smallElements * typeBytes >= ROLL_GM_BLOCK_BYTES) ||
        (!allowWideShiftOnePatch && blockBytes > ROLL_FLAT_PATCH_MAX_BLOCK_BYTES) ||
        (!preferBf16W3SmallPatch && !preferBf16Rank3W19LargePatch &&
            totalBytes < ROLL_FLAT_PATCH_MIN_TOTAL_BYTES)) {
        return false;
    }
    if (firstElements <= secondElements) {
        CopySegment(dstIndex + firstElements, srcIndex, totalElements - firstElements);
        if (!CopyStridedSingleElementByRowGather(
                dstIndex, srcIndex, secondElements, blockSize, blockCount, preferRowGatherPatch)) {
            CopyStridedSourceSegments(dstIndex, srcIndex + secondElements, firstElements, blockSize, blockCount);
        }
    } else {
        CopySegment(dstIndex, srcIndex + secondElements, totalElements - secondElements);
        if (!CopyStridedSingleElementByRowGather(
                dstIndex + firstElements, srcIndex, 0, blockSize, blockCount, preferRowGatherPatch)) {
...
__aicore__ inline void Roll<T>::CopySegment(int64_t dstIndex, int64_t srcIndex, int64_t elementCount)
{
    ...
    const int64_t typeBytes = static_cast<int64_t>(sizeof(T));
    while (copied < elementCount) {
        const int64_t remain = elementCount - copied;
        const int64_t srcByte = (srcIndex + copied) * typeBytes;
        const int64_t dstByte = (dstIndex + copied) * typeBytes;
        if (srcByte % ROLL_GM_BLOCK_BYTES != dstByte % ROLL_GM_BLOCK_BYTES) {
            if (remain * typeBytes >= ROLL_GM_BLOCK_BYTES) {
                CopySegmentBySourceAligned(dstIndex + copied, srcIndex + copied, remain);
                copied += remain;
            } else {
                CopySegmentByScalar(dstIndex + copied, srcIndex + copied, remain);
                copied += remain;
            }
            continue;
        }

        if (dstByte % ROLL_GM_BLOCK_BYTES != 0) {
            const int64_t alignBytes = ROLL_GM_BLOCK_BYTES - dstByte % ROLL_GM_BLOCK_BYTES;
            int64_t current = alignBytes / typeBytes;
            if (current > remain) {
                current = remain;
            }
            CopySegmentByScalar(dstIndex + copied, srcIndex + copied, current);
            copied += current;
            continue;
        }

        if (srcByte % ROLL_GM_BLOCK_BYTES != 0) {
            const int64_t alignBytes = ROLL_GM_BLOCK_BYTES - srcByte % ROLL_GM_BLOCK_BYTES;
            int64_t current = alignBytes / typeBytes;
            if (current > remain) {
                current = remain;
            }
        ...
        int64_t current = remain > ubElements_ ? ubElements_ : remain;
        current = (current * typeBytes / ROLL_GM_BLOCK_BYTES) * ROLL_GM_BLOCK_BYTES / typeBytes;
        if (current <= 0) {
            current = remain > ubElements_ ? ubElements_ : remain;
        }
        const int64_t copyBytes = current * typeBytes;
        if (copyBytes < ROLL_GM_BLOCK_BYTES) {
            CopySegmentByScalar(dstIndex + copied, srcIndex + copied, current);
            copied += current;
            continue;
        }
        ...
        LocalTensor<T> local = inQueue_.AllocTensor<T>();
        DataCopy(local, xGm_[srcIndex + copied], current);
        inQueue_.EnQue(local);
        LocalTensor<T> inLocal = inQueue_.DeQue<T>();

        DataCopy(yGm_[dstIndex + copied], inLocal, current);
        inQueue_.FreeTensor(inLocal);
        copied += current;
    }
}
...
{
    const int64_t total = tilingData_->totalNum;
    const int64_t shift = tilingData_->shifts[0];
    int64_t remain = elementCount_;
    int64_t dst = startIndex_;
    while (remain > 0) {
        const int64_t src = (dst - shift + total) % total;
        int64_t current = remain;
        const int64_t srcContiguous = total - src;
...
__aicore__ inline void Roll<T>::CopyFlattenRollBySource()
{
    ...
    const int64_t shift = tilingData_->shifts[0];
    int64_t remain = elementCount_;
    int64_t src = startIndex_;
    while (remain > 0) {
        int64_t dst = src + shift;
        if (dst >= total) {
            dst -= total;
        }
        int64_t current = remain;
        ...
        const int64_t dstContiguous = total - dst;
        if (current > dstContiguous) {
            current = dstContiguous;
        }
        CopySegmentBySourceAligned(dst, src, current);
        src += current;
        remain -= current;
    }
}
...
{
    const int64_t total = tilingData_->totalNum;
    const int64_t shift = tilingData_->activeShift * tilingData_->innerSize;
    const int64_t typeBytes = static_cast<int64_t>(sizeof(T));
    const bool preferAlignedPlainCopy = IsSameType<T, uint8_t>::value && tilingData_->dimNum > 2 &&
        tilingData_->activeDimCount == 1 && tilingData_->activeDim == 0 &&
        tilingData_->innerSize % ROLL_GM_BLOCK_BYTES == 0 && total >= 32 * 1024 * 1024;
    int64_t remain = elementCount_;
    int64_t src = startIndex_;
    while (remain > 0) {
        int64_t dst = src + shift;
        ...
        const bool alignedCopy = preferAlignedPlainCopy &&
            (src * typeBytes) % ROLL_GM_BLOCK_BYTES == 0 &&
            (dst * typeBytes) % ROLL_GM_BLOCK_BYTES == 0 &&
            current * typeBytes >= ROLL_GM_BLOCK_BYTES;
        if (alignedCopy) {
            CopySegment(dst, src, current);
        } else {
            CopySegmentBySourceAligned(dst, src, current);
...
{
    const int64_t inner = tilingData_->innerSize;
    ...
    int64_t dst = startIndex_;
    while (remain > 0) {
        const int64_t inBlockOffset = dst % blockSize;
        const int64_t blockBase = dst - inBlockOffset;
        int64_t src = 0;
        int64_t contiguous = 0;
        if (inBlockOffset < firstElements) {
            src = blockBase + secondElements + inBlockOffset;
            contiguous = firstElements - inBlockOffset;
        } else {
            src = blockBase + inBlockOffset - firstElements;
...
__aicore__ inline void Roll<T>::CopySingleDimPartial(int64_t& dstIndex, int64_t& remain)
{
    ...
    const int64_t blockBase = dstIndex - inBlockOffset;
    int64_t src = 0;
    int64_t contiguous = 0;
    if (inBlockOffset < firstElements) {
        src = blockBase + secondElements + inBlockOffset;
...
__aicore__ inline void Roll<T>::CopySingleDimFullBlocks(int64_t dstIndex, int64_t blockCount)
{
    ...
    const int64_t firstElements = shift * inner;
    const int64_t secondElements = (dimSize - shift) * inner;
    if (CopyBlockRollByFlatPatch(dstIndex, dstIndex, blockSize, firstElements, secondElements, blockCount)) {
        return;
    }
    CopyStridedSourceSegments(dstIndex, dstIndex + secondElements, firstElements, blockSize, blockCount);
    CopyStridedSourceSegments(dstIndex + firstElements, dstIndex, secondElements, blockSize, blockCount);
}
...
__aicore__ inline void Roll<T>::CopySingleDimRollByBlocks()
{
    ...
    const int64_t typeBytes = static_cast<int64_t>(sizeof(T));
    const int64_t blockBytes = blockSize * typeBytes;
    const int64_t alignedBlockBytes =
        ((blockBytes + ROLL_GM_BLOCK_BYTES - 1) / ROLL_GM_BLOCK_BYTES) * ROLL_GM_BLOCK_BYTES;
    const int64_t alignedBlockElements = alignedBlockBytes / typeBytes;
    if (inner <= 0 || dimSize <= 1 || blockSize <= 0 || alignedBlockElements <= 0 ||
        alignedBlockElements > ubElements_) {
        CopySingleDimRoll();
        return;
    }

    const int64_t maxInputBlocks = ubElements_ / alignedBlockElements;
    const int64_t maxOutputBlocks = ubElements_ / blockSize;
    int64_t maxBlocks = maxInputBlocks < maxOutputBlocks ? maxInputBlocks : maxOutputBlocks;
    if (maxBlocks <= 0) {
        CopySingleDimRoll();
        return;
    }
    ...
    while (fullBlocks > 0) {
        int64_t currentBlocks = fullBlocks > maxBlocks ? maxBlocks : fullBlocks;
...
__aicore__ inline void Roll<T>::CopyLastDimRoll()
{
    ...
    while (remain > 0) {
        ...
        const int64_t rowBase = dst - rowOffset;
        const int64_t srcOffset = (rowOffset - shift + dimSize) % dimSize;
        int64_t current = remain;
        ...
        CopySegment(dst, rowBase + srcOffset, current);
        dst += current;
        remain -= current;
    }
}
...
__aicore__ inline void Roll<T>::CopyLastDimPartial(int64_t& dstIndex, int64_t& remain)
{
    ...
    const int64_t rowBase = dstIndex - rowOffset;
    const int64_t srcOffset = (rowOffset - shift + dimSize) % dimSize;
    int64_t current = remain;
    const int64_t dstContiguous = dimSize - rowOffset;
    if (current > dstContiguous) {
    ...
    CopySegment(dstIndex, rowBase + srcOffset, current);
    dstIndex += current;
    remain -= current;
}
...
__aicore__ inline void Roll<T>::CopyRowsRollInUb(
    ...
    int64_t alignedRowElements, int64_t shift)
{
    const int64_t firstSrcOffset = dimSize - shift;
    const int64_t secondCount = dimSize - shift;
    if (tilingData_->useSafeUbShuffle != 0 || dimSize <= 8 || IsSameType<T, bool>::value) {
        for (int64_t row = 0; row < rowCount; ++row) {
            const int64_t srcBase = row * alignedRowElements;
            const int64_t dstBase = row * dimSize;
            for (int64_t i = 0; i < shift; ++i) {
                outLocal.SetValue(dstBase + i, inLocal.GetValue(srcBase + firstSrcOffset + i));
            }
            for (int64_t i = 0; i < secondCount; ++i) {
                outLocal.SetValue(dstBase + shift + i, inLocal.GetValue(srcBase + i));
            }
        }
        return;
    }
    ...
    for (int64_t row = 0; row < rowCount; ++row) {
        ...
        const int64_t dstBase = row * dimSize;
        for (int64_t i = 0; i < shift; ++i) {
            outPtr[dstBase + i] = inPtr[srcBase + firstSrcOffset + i];
        }
        for (int64_t i = 0; i < secondCount; ++i) {
            outPtr[dstBase + shift + i] = inPtr[srcBase + i];
        }
    }
}
...
__aicore__ inline void Roll<T>::CopyLastDimFullRows(int64_t dstIndex, int64_t rowCount)
{
    ...
    copyInParams.dstStride = 0;
    DataCopyPadExtParams<T> padParams{false, 0, 0, static_cast<T>(0)};
    DataCopyPad(local, xGm_[dstIndex], copyInParams, padParams);
    inQueue_.EnQue(local);
    LocalTensor<T> inLocal = inQueue_.DeQue<T>();
...
__aicore__ inline void Roll<T>::CopyLastDimFullRowsBySegments(int64_t dstIndex, int64_t rowCount)
{
    ...
    if (CopyBlockRollByFlatPatch(dstIndex, dstIndex, dimSize, firstElements, secondElements, rowCount)) {
        return;
    }
    CopyStridedSourceSegments(dstIndex, dstIndex + secondElements, firstElements, dimSize, rowCount);
    CopyStridedSourceSegments(dstIndex + firstElements, dstIndex, secondElements, dimSize, rowCount);
}
    ...
    while (fullRows > 0) {
        ...
        if (tilingData_->dimNum == 2) {
            ...
            if (IsSameType<T, bfloat16_t>::value &&
                ...
                rowShift != 0 &&
                shift != 0 &&
                tilingData_->totalNum * static_cast<int64_t>(sizeof(T)) >= 512 &&
...
__aicore__ inline void Roll<T>::CopyLastDimRollByRows()
{
    ...
    while (fullRows > 0) {
        ...
        const int64_t largeSegmentBytes = (shift > tailElements ? shift : tailElements) * typeBytes;
        int64_t currentRows = fullRows > maxRows ? maxRows : fullRows;
        if (IsSameType<T, uint8_t>::value && dimSize >= 1024 && dimSize <= ubElements_ &&
            (shift == 1 || shift == dimSize - 1)) {
            currentRows = fullRows > ROLL_MAX_DATACOPY_BLOCK_COUNT ? ROLL_MAX_DATACOPY_BLOCK_COUNT : fullRows;
        }
        const bool preferSegmentPath = (largeSegmentBytes >= ROLL_STRIDED_SEGMENT_MIN_BYTES) ||
            (IsSameType<T, uint8_t>::value && dimSize >= 7 && dimSize <= 31);
...
__aicore__ inline void Roll<T>::Process()
{
    ...
    if (tilingData_->activeDimCount == 0) {
        ...
        tilingData_->activeDimCount == 1 && tilingData_->activeDim == 0 && tilingData_->innerSize > 0 &&
        ...
        tilingData_->activeDimCount == 1 && tilingData_->innerSize == 1 &&
