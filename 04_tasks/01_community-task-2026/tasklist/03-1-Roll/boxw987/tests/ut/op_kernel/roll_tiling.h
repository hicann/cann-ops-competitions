...
/*!
 * \file roll_tiling.h
 * \brief Roll kernel UT tiling shim.
 */

#ifndef ROLL_UT_TILING_H_
#define ROLL_UT_TILING_H_
...
#include "../../../op_kernel/roll.h"

#define __aicore__
#ifdef __NPU_TILING__
inline[aicore] void InitTilingData(const __gm__ uint8_t *tiling, RollTilingData *constData)
{
    const __gm__ uint32_t *src = reinterpret_cast<const __gm__ uint32_t *>(tiling);
    uint32_t *dst = reinterpret_cast<uint32_t *>(constData);
    for (size_t i = 0; i < sizeof(RollTilingData) / sizeof(uint32_t); ++i) {
        *(dst + i) = *(src + i);
    }
}
#else
