...
- 全部为 Host：走 CPU 直接计算；
- 全部为 Device：获取当前 handle 绑定的 stream，启动一个 Ascend C SIMT kernel；
- Host/Device 混合：返回 `ACLBLAS_STATUS_INVALID_VALUE`。

这里必须直接复用当前 ops-blas `srotg_host.cpp` 已有的指针属性判定/launch helper，
不能自行猜测私有 kernel launch API。
...
本版本不再使用简单的 `scale=max(|Re|,|Im|)` 单级归一化作为最终实现。
Netlib `crotg` 的 Algorithm 978 实际包含“可安全直接计算”和“缩放计算”两级路径，并在
`f1/u < sqrt(safmin)` 时进一步分别缩放 `f`、`g`，最后再恢复 `c` 和 `r`。
实现按 Netlib `crotg.f90` 的分支结构复现，确保极端数量级下既避免溢出，也避免
`f2/h2`、`h2/f2` 等中间量异常。

对 IEEE754 float32，采用与 Netlib 相同语义的：
` safmin = 2^-125`、`safmax = 2^127`、`rtmin = sqrt(safmin)`。

当 `scale == 0` 时按照 `a=b=0` 的特殊规则直接返回：
...
若 f、g 均处于安全无缩放范围：
    ...
    c = sqrt(f2/h2)
    r = f/c
    s = conj(g) * (f/sqrt(f2*h2)) 或等价安全形式

否则：
    u = clamp(max(f1,g1), safmin, safmax)
    g = g/u
    当 f/u 过小，再以 v=f1 的安全尺度单独缩放 f
    在缩放域中计算 f2、g2、h2、c、r、s
    最后恢复 c *= w、r *= u
```

代码实现必须保留上述分支，不允许将其重新简化成一次 `scale` 除法 + `sqrt`。
...
7. 本算子一次调用只有一个逻辑计算实例，不存在跨元素并行归约；不额外要求多核确定性。
...
测试程序必须显式输出每个性能 case 的 `avg_us`；不能使用 GTest 汇总行的整数 `ms`
作为标量算子性能指标。
...
2026-09-11 在 Ascend 950PR 环境完成 `crotg_test` 实测：72 个 GTest，45 PASS、27 SKIPPED、0 FAIL，最终返回 `[PASS] crotg_test`。其中 5 个性能 case 在 `PerfDevice` 路径实际执行，并输出 `CROTG_PERF ... avg_us=...`。

### 8.2 数学性质验证要求

除逐分量 golden 外，最终测试报告应输出以下误差：
...
其中测试计算必须使用原始输入 `a0/b0`，不能使用已经被原地覆写后的 `a` 作为验证输入。

### 8.3 内存说明

任务书 §3.4 规定本算子“内存要求：不涉及”。设计上不申请 workspace、不创建额外 Tensor、不做数据动态切分。正式验收表若要求内存数据，应使用实际 profiler 结果，不以理论值冒充实测值。

### 8.4 特殊值验收

任务用例集合包含 Inf/NaN。最终测试摘要应明确列出这些 case 的实际执行状态及 verdict。若某平台/编译路径对非有限值有不同处理，应以实际测试工程和任务指定参考语义为准。

### 8.5 最终交付项

最终交付闭环包含：

1. `blas/rotg/arch35/` 算子实现；
2. `test/rotg/crotg/arch35/` 测试代码与 CSV；
3. `blas/rotg/README.md` 产品支持表；
4. `include/cann_ops_blas.h` 公共 API 声明；
5. 设计文档 PR；
6. 自测报告；
7. 个人仓库/分支/目录及 `Ascend-CANN` 开发者权限；
8. ops-blas 代码 PR 与 CI 通过。
