...
## 3.9 设计修订（2026-09-03 二版：热路径改走硬件 Sort 原语）

评审通过后上板实测与攻关中，排序热路径由"纯标量归并"升级为"硬件 `Sort<float>`
全排"，3.3.2/3.3.3 的归并路径降级为兜底。修订点如下（代码 PR：cann/ops-sparse
!170 顶端提交）：

### 3.9.1 核心排序原语：唯一 float 合成键 + 硬件 Sort 降序全排

arch22 的 AscendC `Sort` 仅支持 half/float 键且为纯降序，直接用行号做键无法得到
稳定升序。为此构造**段内唯一**的合成键：
...
- `segMaxRow` 为段内实测最大行号；key 为 < 2^24 的非负精确整数（float32 尾数
  精确上限），段内唯一 ⇒ 降序全排等价于行号升序、同行按原始位置升序（稳定），
  排序结果完全确定，不依赖硬件 tie 行为；
- pad 槽位键置 -1.0f（小于一切真实键，降序沉底）；Sort 的 index 通道预填 iota，
  `Extract` 解交织出每秩源下标，`ShiftLeft` 转字节偏移后 `Gather` 同步重排
  rowInd/P；
- iota 用 int32 版 `CreateVecIndex`（uint32 版内部 Adds 不被 A2 支持，非负值
  位型等价）。

**行域上界的两种来源**（ Correctness 与性能兼顾 ）：

- host 传入真实 m（< 2^25）：直接以 m-1+indexBase 为行域上界，零额外开销；
- m 为占位值（如 torch 验收 hook 无 rows 入参）：kernel 内 `ReduceMax` 逐段
  实测 segMaxRow（V_S 屏障读回标量），位宽 segRowBits+posBits > 24 的段回退
  3.3.2 的标量归并（正确性兜底，绝不静默产生错误结果）。

### 3.9.2 短列/长列路径调整

- **短列批量化保留**，槽位由 8 元素对齐改为 32 元素对齐（Sort 以 repeat×32
  整体处理）；**len < 64 的超短段仍走标量归并**——实测矢量 Sort 单段固定开销
  （十余条矢量指令 + Sort 内部 PipeBarrier）约 26μs，在 P-01 量级（~18 元素/列）
  反而慢于标量归并，交叉点约 32 元素，阈值取 64 留裕量；
- **长列**：分块 UB 排序（同矢量原语）写入 GM workspace 形成有序 run 后多趟
  归并。**归并的 GM 写全部改为 UB 分块暂存 + DataCopyPad 下刷**，消除标量 GM
  写——实测发现标量 GM 写与其他核 MTE3 写在共享 32B 扇区上会互相踩回旧数据
  （列边界非 128B 对齐时，核间一致性风险），归并输入为标量 GM 只读不受影响。

### 3.9.3 修订后实测（官方 benchmark 口径：预热 10 次、采样 30 次取中位数）

| 锚点 | 场景 | A3(910C) 实测（2026-09-04） | A2(910B3) 实测（2026-09-04） | GPU 标杆 median_us | A3 倍率 | A2 倍率 | 门槛 |
|------|------|---------------|----------------|--------------------|---------|---------|------|
| P-01 | 8192×28672，nnz=524288 | 5.95 ms | 5.64 ms | 2961.7–2969.3 μs | **0.50×** | **0.52×** | ≥0.25× |
| P-02 | 4096×1536，nnz=262144 | 2.47 ms | 2.45 ms | 2595.9–2610.4 μs | **1.05×** | **1.06×** | ≥0.25× |
| P-03 | 7168×2048，nnz=458752 | 3.33 ms | 3.26 ms | 2780.8–2783.1 μs | **0.83×** | **0.85×** | ≥0.25× |

双平台三锚点全部达标（最小余量 P-01 为门槛的 2.0 倍），P-02 双平台均超过 GPU
标杆；A3 全量双遍复跑偏差 < 0.5%，锚点 6 case（base0/base1）单独重跑与全量结果
一致。精度 200/200（exact match）与 gtest 49/49 双平台全绿。msprof 内核画像
（`cscsort_kernel` 全程 AI_VECTOR_CORE，无 CPU fallback）与结果文件归档于
fork 仓 `evidence/a3-20260903-v2` 分支。
