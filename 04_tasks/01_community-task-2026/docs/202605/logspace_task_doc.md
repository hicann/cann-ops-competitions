...
当前aclnnlogspace算子不支持int16,int32,int8,uint8数据类型，需要在原来的代码上进行再开发，使其支持int16,int32,int8,uint8数据类型。要求完成算子设计，算子开发及算子测试任务。验收后需将该算子提交到算子开源仓。
aclnnlogspace调用的是AscendC算子linspace，可在其功能上扩展。

## 核心开发要求及验收标准

### 功能实现要求

1. 支持int16,int32,int8,uint8等数据类型。注意还应修改相应文档，补充支持情况。
2. 必须实现算子泛化功能，满足各类合法输入场景的计算需求，验收阶段将采用泛化数据进行验收。

### 参数说明
**当前不支持int16,int32,int8,uint8**
| 参数名 | 输入/输出/属性 | 描述 | 使用说明 | 数据类型 | 数据格式 | 维度(shape) | 非连续Tensor |
|--------|----------------|------|----------|----------|----------|-------------|--------------|
| start (aclScalar*) | 输入 | 表示LogSpace的第一个输入，对数序列的起始指数。 | - | FLOAT、FLOAT16、BFLOAT16 | ND | - | √ |
| end (aclScalar*) | 输入 | 表示LogSpace的第二个输入，对数序列的结束指数。 | - | FLOAT、FLOAT16、BFLOAT16 | ND | - | √ |
| steps (int64_t) | 输入 | 序列中的元素数量。 | - | int64_t | - | - | - |
| base (double) | 输入 | 对数空间的底数。 | - | double | - | - | - |
| out (aclTensor*) | 输出 | 表示LogSpace的输出，输出的对数间隔序列张量。 | - | FLOAT、FLOAT16、BFLOAT16 | ND | 2-8 | √ |
...
新增的int16,int32,int8,uint8等数据类型，性能不劣于原aclnnlogspace float32数据类型的性能
...
验收通过后，在昇腾算子开源仓提交 PR 申请，申请将开发完成的算子合入 [https://gitcode.com/cann/ops-math/tree/master/experimental/math](https://gitcode.com/cann/ops-math/tree/master/experimental/math)。
