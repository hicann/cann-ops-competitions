...
参考昇腾版本内置aclnnInplaceRsqrt算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子,完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 与原 TBE 算子核心功能完全对齐，支持float32, float16, bfloat16, bool, int8, int16, uint8, int32
...
|--------|----------------|------|----------|----------|----------|-------------|--------------|
| selfRef（aclTensor*） | 输入/输出 | 公式中的input。 | 支持空Tensor。 | float32、float16、bfloat16、bool、int8、int16、uint8、int32 | ND | 0-8 | √ |
