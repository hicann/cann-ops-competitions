...
参考昇腾版本内置aclnnFmodScalar aclnnFmodTensor算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子，完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 与原 TBE 算子核心功能完全对齐，支持BFLOAT16、FLOAT16、FLOAT32、INT16等4种数据类型。
...
|--------|----------------|------|----------|----------|----------|-------------|--------------|
| self（aclTensor*） | 输入 | 待进行Fmod的入参。 | - | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | 0-8 | √ |
| other（aclScalar*） | 输入 | 待进行Fmod的入参。 | - | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | - | - |
| out（aclTensor*） | 输出 | 待进行Fmod的出参。 | 数据类型需要是self与other推导之后可转换的数据类型，shape需要与self一致。 | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | 0-8 | - |

  **aclnnFmodTensor**
| 参数名 | 输入/输出/属性 | 描述 | 使用说明 | 数据类型 | 数据格式 | 维度(shape) | 非连续Tensor |
|--------|----------------|------|----------|----------|----------|-------------|--------------|
| self（aclTensor*） | 输入 | 待进行Fmod计算的入参，公式中的self。 | - | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | 0-8 | √ |
| other（aclTensor*） | 输入 | 待进行Fmod计算的入参，公式中的other。 | - | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | 0-8 | √ |
| out（aclTensor*） | 输出 | 待进行Fmod计算的出参，公式中的out。 | 数据类型需要是self与other推导之后可转换的数据类型，shape需要与self一致。 | BFLOAT16、FLOAT16、FLOAT32、INT16 | ND | 0-8 | √ |
...
1. kernel 实现：`/usr/local/Ascend/ascend-toolkit/latest/opp/built-in/op_impl/ai_core/tbe/impl/ops_legacy/dynamic/mod.py`
2. 算子原型：`/usr/local/Ascend/ascend-toolkit/latest/opp/built-in/op_proto/inc/ops_proto_math.h`
...
3. 代码样例：[https://gitcode.com/cann/ops-math/tree/master/math/mod](https://gitcode.com/cann/ops-math/tree/master/math/mod)
