...
当前aclnnIndexFillTensor算子不支持int16, int8, uint8，double数据类型，需要在原来的代码上进行再开发，使其支持int16, int8, uint8，double数据类型。要求完成算子设计，算子开发及算子测试任务。验收后需将该算子提交到算子开源仓。
已有AscendC算子IndexFill，可以在indexFill算子的代码上进行再开发，使其支持int16, int8, uint8，double数据类型，然后aclnnIndexFillTensor接口再调用IndexFill算子。

## 核心开发要求及验收标准

### 功能实现要求

1. 扩展支持支持int16, int8, uint8，double数据类型。注意还应修改相应文档，补充支持情况。
2. 必须实现算子泛化功能，满足各类合法输入场景的计算需求，验收阶段将采用泛化数据进行验收。

### 参数说明
**当前未支持int16, int8, uint8，double类型**

| 参数名 | 输入/输出/属性 | 描述 | 使用说明 | 数据类型 | 数据格式 | 维度(shape) | 非连续Tensor |
|---|---|---|---|---|---|---|---|
| self | 输入 | 输入Tensor。 | 待被在指定位置的值用value替换的张量 | FLOAT16、FLOAT、INT32、INT64、BOOL、BFLOAT16 | ND | [1,8] | √ |
| dim | 属性 | 指定了self将要填充的维度。 | dim的取值范围在[-self.dim(), self.dim())，当self为0维时，dim的取值范围在[-1, 1) | int64 | - | - | - |
| index | 输入 | 指定self在dim维度将要填充的下标。 |其中的元素值小于self对应dim的维度大小 | INT32、INT64 | - | - | - |
| value | 输入 | 指定填充的数据值。 | 需要可转化为self的数据类型。 | FLOAT16、FLOAT、INT32、INT64、BOOL、BFLOAT16 | - | - | - |
...
> int16, int8, uint8不劣化于int32数据类型。double类型不劣化于INT64类型
...
验收通过后，在昇腾算子开源仓提交 PR 申请 [https://gitcode.com/cann/ops-nn/tree/master/experimental/index](https://gitcode.com/cann/ops-nn/tree/master/experimental/index)。
