...
1. 算子整体性能需与0.8倍GPU（A100）持平，StrsmBatched和CtrsmBatched分别独立进行性能测试，两个算子均需通过性能指标要求。
2. 性能测试用例设计：
    - batchCount范围[1, 256]内随机采样;
    - m，n范围[16, 8192]内随机采样；
    - side，uplo，transa，diag进行排列组合；
    - 测试用例保证为合法输入。
3. 性能比对方法：
    - 对于同一用例，统计GPU执行结果t1和NPU执行结果t2，计算耗时的比值t1/t2;
    - 执行所有用例，计算所有耗时比值的平均值Mean(t1/t2)；
    - 耗时比值的平均值Mean(t1/t2)>=0.8。
