...
参考昇腾版本内置aclnnGcd算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子。完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 与原 TBE 算子核心功能完全对齐，支持fp32/fp16/bf16及int8, uint8, int16等类型。
...
|--------|----------------|------|----------|----------|----------|-------------|--------------|
| self（aclTensor*） | 输入 | 表示待转换的目标张量。 | shape需与other满足broadcast关系 | fp32、fp16、bf16、int8、uint8、int16 | ND | 1-8 | √ |
| other（aclTensor*） | 输入 | 表示待转换的目标张量。 | shape需与self满足broadcast关系。 | fp32、fp16、bf16、int8、uint8、int16 | ND | 1-8 | √ |
| out（aclTensor*） | 输出 | self和other求最大公约数的结果。 |shape需与self和other进行broadcast后的shape一致。 | fp32、fp16、bf16、int8、uint8、int16 | ND | 1-8 | √ |
...
验收通过后，在昇腾算子开源仓提交 PR 申请，申请将开发完成的算子合入https://gitcode.com/cann/ops-math/tree/master/experimental/math。
...
4. 已有算子实现参考：https://gitee.com/ascend/cann-ops/tree/master/src/math/gcd
