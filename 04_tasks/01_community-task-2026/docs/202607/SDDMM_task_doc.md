...
满足《[生态算子开源精度标准](https://gitcode.com/cann/opbase/blob/master/docs/zh/ops_precision_standard/experimental_standard.md)》；使用 [ATK](https://gitcode.com/Ascend/ATK) 进行**单标杆**精度验收，以 CPU 高精度结果为 golden，对 NPU（aclsparse）输出进行误差评估。测试用例及测试工程见[测试目录](./self_test_case/sddmm/)的 `atk_test/aclSparseSddmm/result/aclSparseSddmm/json/all_aclSparseSddmm.json`（至少完成 200 个 case 测试）。
