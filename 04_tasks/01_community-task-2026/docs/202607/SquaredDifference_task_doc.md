# CANN训练营2026暑期季 – 西北工业大学专场-SquaredDifference算子开发任务书
...
参考昇腾版本内置SquaredDifference算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子，完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 与原算子核心功能完全对齐，支持原算子对应的所有数据类型、数据格式。
2. 必须实现算子泛化功能，满足各类合法输入场景的计算需求，验收阶段将采用泛化数据进行验收。
3. 支持广播功能，输入 x1 和 x2 的 shape 需满足广播规则。
4. 计算公式：$y = (x1 - x2) * (x1 - x2) = (x1 - x2)^2$，为第一个输入张量减去第二个输入张量，并计算其平方值。

### 测试标准

需参考内置算子自行设计全场景自验证用例，验收阶段将采用泛化数据进行功能、精度、性能全维度验证。自验证报告完整、可复现，所有测试用例执行通过。

### 性能要求

1. 暂仅要求**所有核参与计算场景**下，性能不低于原算子的 95%。
2. 如小shape无法达标（10us以下场景相差3us），提供性能仿真图和分析结论证明Ascend C实现与原实现完全一致或优于原实现。
...
3. 代码样例：[https://gitcode.com/cann/ops-math/tree/master/experimental/math/bitwise_and](https://gitcode.com/cann/ops-math/tree/master/experimental/math/bitwise_and)
