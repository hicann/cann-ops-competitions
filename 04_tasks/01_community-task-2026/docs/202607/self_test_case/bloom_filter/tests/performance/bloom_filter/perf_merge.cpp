...
template <typename Key, typename Word, int BlockBits, int PatternBits, int HorizontalLayout, int VerticalLayout>
MergeTestContext<Key, Word, BlockBits, PatternBits, HorizontalLayout, VerticalLayout>& GetContext() {
    static MergeTestContext<Key, Word, BlockBits, PatternBits, HorizontalLayout, VerticalLayout> ctx;
    return ctx;
}

template <typename Key, typename Word, int BlockBits, int PatternBits, int HorizontalLayout, int VerticalLayout>
void SetupMergeTest(int numKeys, int filterSizeMB) {
...
TestResult TestMerge() {
    ...
    ctx.filterA->Merge(*ctx.filterB, ctx.stream);
...
REGISTER_PERFORMANCE_TEST(mergeCase1, (TestMerge<int32_t, uint32_t, 256, 8, 1, 8>), (SetupMergeTest<int32_t, uint32_t, 256, 8, 1, 8>), int, int);
REGISTER_PERFORMANCE_TEST(mergeCase2, (TestMerge<int64_t, uint32_t, 256, 8, 1, 8>), (SetupMergeTest<int64_t, uint32_t, 256, 8, 1, 8>), int, int);

REGISTER_PERFORMANCE_ARGS(mergeCase1, "merge_int32",
    (std::initializer_list<std::tuple<int,int>>{
        {80000000, 32}, {80000000, 256}, {80000000, 2048}
    }), int, int);
REGISTER_PERFORMANCE_ARGS(mergeCase2, "merge_int64",
    (std::initializer_list<std::tuple<int,int>>{
        {80000000, 32}, {80000000, 256}, {80000000, 2048}
    }), int, int);
