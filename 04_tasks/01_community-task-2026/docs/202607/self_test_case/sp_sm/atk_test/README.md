...
aclSparse **SpSM** 的 ATK 精度验收工程（NPU 被测，CPU golden 单标杆）。
...
└── aclSparseSpsm/
    ├── aclSparseSpsm.yaml
    ├── generator_aclSparseSpsm.py
    ├── executor_aclSparseSpsm.py
    └── result/aclSparseSpsm/json/all_aclSparseSpsm.json
```
...
```bash
cd self_test_case/sp_sm/atk_test

# NPU 精度验收（推荐）
./run_sparse_atk.sh spsm npu

# 切片用例
./run_sparse_atk.sh spsm npu -s 0 -e 10
```
...
- 预置 200 条：`aclSparseSpsm/result/aclSparseSpsm/json/all_aclSparseSpsm.json`
- 用例定义：`generator_aclSparseSpsm.py` + `aclSparseSpsm.yaml`
- 生成：`atk case -f aclSparseSpsm.yaml -p generator_aclSparseSpsm.py`
