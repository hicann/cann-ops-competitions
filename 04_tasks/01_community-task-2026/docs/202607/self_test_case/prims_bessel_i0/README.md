...
本用例用于验证BesselI0高阶API的功能与性能。BesselI0按元素计算第一类零阶修正贝塞尔函数（modified Bessel function of the first kind of order 0）。

$$\mathrm{I}_0(x) = \sum_{k=0}^{\infty} \frac{1}{(k!)^2}\left(\frac{x}{2}\right)^{2k}$$

> **说明：** 本用例以Erf高阶API进行功能与性能验证（kernel 内调用 `AscendC::Erf`，golden 对齐 `scipy.special.erf`）。待 BesselI0 API 开发完成后，将 `.asc` 中的 `AscendC::Erf` 替换为 `AscendC::BesselI0`、`gen_data.py` 的 golden 替换为 `scipy.special.i0` 即可。
...
│   ├── bessel_i0.asc           // Ascend C高阶API用例实现
...
- 接口功能：按元素计算第一类零阶修正贝塞尔函数I0(x)。
  $$dstTensor_i = \mathrm{I}_0(srcTensor_i)$$
- 接口规格：

  <table>
  <tr><td rowspan="1" align="center">用例类型</td><td colspan="4" align="center">BesselI0Custom</td></tr>
  ...
  <tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">bessel_i0_custom</td></tr>
  </table>
...
CopyIn → Compute(BesselI0) → CopyOut，输出与真值对比验证精度。
