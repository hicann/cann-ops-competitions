...
template <typename Key, typename Word, int BlockBits, int PatternBits, int HorizontalLayout, int VerticalLayout>
CreateTestContext<Key, Word, BlockBits, PatternBits, HorizontalLayout, VerticalLayout>& GetContext() {
    static CreateTestContext<Key, Word, BlockBits, PatternBits, HorizontalLayout, VerticalLayout> ctx;
    return ctx;
}

template <typename Key, typename Word, int BlockBits, int PatternBits, int HorizontalLayout, int VerticalLayout>
void SetupCreateTest(int filterSizeMB) {
...
TestResult TestCreate() {
    ...
    auto filter = bloom_filter_factory::MakeBloomFilter<Key>(numBlocks, ctx.stream);
    Sync(ctx.stream);
...
REGISTER_PERFORMANCE_TEST(createCase1, (TestCreate<int32_t, uint32_t, 256, 8, 8, 1>), (SetupCreateTest<int32_t, uint32_t, 256, 8, 8, 1>), int);
REGISTER_PERFORMANCE_TEST(createCase2, (TestCreate<int64_t, uint32_t, 256, 8, 8, 1>), (SetupCreateTest<int64_t, uint32_t, 256, 8, 8, 1>), int);

REGISTER_PERFORMANCE_ARGS(createCase1, "create_int32",
    (std::initializer_list<std::tuple<int>>{
        {32}, {256}, {2048}
    }),
    int);

REGISTER_PERFORMANCE_ARGS(createCase2, "create_int64",
    (std::initializer_list<std::tuple<int>>{
        {32}, {256}, {2048}
    }),
    int);
