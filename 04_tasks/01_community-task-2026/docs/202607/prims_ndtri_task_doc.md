...
基于 PyTorch IR 的 `prims.ndtri` 接口，在昇腾 NPU 上使用 Ascend C 编程语言实现对应的高阶API。该API用于计算标准正态分布的逆累积分布函数（分位数函数），输入概率值p，输出对应的z值使得P(Z≤z)=p，完成API设计、开发、测试全流程工作，测试通过后将代码提交至asc-devkit开源仓。
