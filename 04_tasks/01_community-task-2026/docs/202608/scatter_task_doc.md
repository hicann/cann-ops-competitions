...
- 以下分档指标为 **达标基线（最低验收门槛）**，以 **GPU A100** 上 torch_scatter 实测耗时为 100% 参照；
- 开发过程中可探索多种实现路径（如原子归约、分桶归约、私有化直方图、SIMT 路径等），**验收与合入取各路径中性能最优的实现**；
- 自验证报告须列出各候选方案对比数据，并说明最终选用方案及理由；
- **float64/int64（L2 路径）不做性能考核**。

#### 硬件参照（Ascend 950PR vs A100 80GB SXM）

| 指标 | A100 80GB SXM | Ascend 950PR (Atlas 350) | 对 scatter 的影响 |
|------|---------------|---------------------------|-------------------|
| HBM 峰值带宽 | 2.0 TB/s | 1.4～1.6 TB/s | scatter 为访存密集型，带宽是主要上限 |
| FP32 峰值算力 | 19.5 TFLOPS | ~30 TFLOPS (Vector) | scatter 算术强度极低，算力非瓶颈 |
| 访存颗粒度 | ~128B | **128B Sector**（由 512B 优化） | 950PR 对离散/小包 scatter 更友好 |
| FP64 原生 | 9.7 TFLOPS | **无** | float64 走 L2 可选路径 |

#### 分档性能达标基线（相对 A100，NPU L1 dtype）

| 场景分档 | 特征 | reduce | dtype | **达标基线（≥）** |
|----------|------|--------|-------|-------------------|
| **S1 GNN 典型** | 10⁴～10⁵ 节点，中等 index 冲突 | sum | float32 | **0.50×** |
| **S2 GNN 典型** | 同上 | mean | float32 | **0.45×**（相对 A100 mean） |
| **S3 低冲突** | index 分布均匀，feature 维较大 | sum | float32 | **0.50×** |
| **S4 高冲突** | 大量边指向少量节点（index 重复率高） | sum | float32 | **0.35×** |
| **S5 低精度** | GNN 常规负载 | sum | float16/bfloat16 | **0.55×** |
| **S6 广播** | 1D index 广播到高维 src | sum | float32 | **0.50×** |
| **S7 min/max** | 含 arg_out 两阶段 | min/max | float32 | **0.45×** |
| **S8 L2 路径** | float64 / int64 | 全部 | float64/int64 | **不做性能考核**（功能+精度必过） |

> **加权平均目标**：在 S1/S3/S4/S5 混合负载下，NPU L1 dtype 的 `reduce=sum, float32` **平均达到 ≥ 0.50× A100** 即为性能达标；最终合入代码须为实测最优方案。

#### 验收基准用例（须实测填列）

以下用例用于填充 A100 基线与 NPU 实测比值（`reduce=sum` 除非注明）：

| 编号 | 场景描述 | src shape | index 规模 | dim | 分档 | GPU A100 参考 | NPU 达标基线 |
|------|----------|-----------|------------|-----|------|---------------|--------------|
| P-01 | GNN 小规模聚合 | `[10000, 16, 64]` | `[10000, 16]` | 0 | S1 | 待实测填入 | ≥ 0.50× |
| P-02 | GNN 中规模 | `[100000, 32, 128]` | `[100000, 32]` | 0 | S1 | 待实测填入 | ≥ 0.50× |
| P-03 | 高冲突 scatter | `[500000, 1]` | `[500000]`，index∈[0,999] | 0 | S4 | 待实测填入 | ≥ 0.35× |
| P-04 | 3D 广播 scatter | `[4, 3, 8, 8]` | `[8]` 广播 | 2 | S6 | 待实测填入 | ≥ 0.50× |
| P-05 | mean 归约 | `[100000, 16, 64]` | `[100000, 16]` | 0 | S2 | 待实测填入 | ≥ 0.45× |
| P-06 | float16 GNN | `[100000, 32, 128]` | `[100000, 32]` | 0 | S5 | 待实测填入 | ≥ 0.55× |
| P-07 | min + arg_out | `[100000, 32, 128]` | `[100000, 32]` | 0 | S7 | 待实测填入 | ≥ 0.45× |

> 开发者提交验收前，须使用 torch_scatter GPU（A100）基准测试填充「GPU A100 参考」列，在自验证报告中给出各用例 NPU/A100 比值，并标注是否达到对应分档基线。
