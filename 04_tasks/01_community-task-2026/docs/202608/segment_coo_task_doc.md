...
按有序索引分组归约，同组元素连续排列。数据：随机矩阵（N行 × 128特征），均分到 seg 段。

#### sum

| shape (N × 128, seg段) | float32 | float16 | 总元素 |
|-------|---------|---------|--------|
| N=256K seg=16K | 0.205ms | 0.247ms | 33M |
| N=512K seg=32K | 0.400ms | 0.482ms | 66M |
| N=1024K seg=32K | 0.743ms | 0.820ms | 131M |
| N=2048K seg=64K | 1.471ms | 1.627ms | 262M |
| N=4096K seg=64K | 2.804ms | 2.951ms | 524M |

#### mean

| shape (N × 128, seg段) | float32 | float16 | 总元素 |
|-------|---------|---------|--------|
| N=256K seg=16K | 0.236ms | 0.280ms | 33M |
| N=512K seg=32K | 0.445ms | 0.532ms | 66M |
| N=1024K seg=32K | 0.799ms | 0.886ms | 131M |
| N=2048K seg=64K | 1.572ms | 1.741ms | 262M |
| N=4096K seg=64K | 2.947ms | 3.133ms | 524M |

#### min

| shape (N × 128, seg段) | float32 | float16 | 总元素 |
|-------|---------|---------|--------|
| N=256K seg=16K | 0.535ms | 0.547ms | 33M |
| N=512K seg=32K | 1.041ms | 1.069ms | 66M |
| N=1024K seg=32K | 1.857ms | 1.878ms | 131M |
| N=2048K seg=64K | 3.690ms | 3.719ms | 262M |
| N=4096K seg=64K | 6.886ms | 6.887ms | 524M |

#### max

| shape (N × 128, seg段) | float32 | float16 | 总元素 |
|-------|---------|---------|--------|
| N=256K seg=16K | 0.534ms | 0.546ms | 33M |
| N=512K seg=32K | 1.040ms | 1.068ms | 66M |
| N=1024K seg=32K | 1.856ms | 1.877ms | 131M |
| N=2048K seg=64K | 3.689ms | 3.719ms | 262M |
| N=4096K seg=64K | 6.885ms | 6.886ms | 524M |
