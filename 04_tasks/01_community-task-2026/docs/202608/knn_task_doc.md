...
KNN 为计算密集型（空间搜索），NPU 实现难度较高。

| 分档 | 场景 | **达标基线（≥ A100）** |
|------|------|------------------------|
| S1 | N=M=10⁴, F=32, k=8 | **0.45×** |
| S2 | N=10⁵, M=10⁴, F=64, k=16 | **0.35×** |
| S3 | cosine=True | **0.40×** |
| S4 | knn_graph 自环剔除 | 同 knn 基线 |
| S5 | float16 | **0.50×** |

验收取最优实现（如 tile-based grid、brute-force SIMT、分块 KD 等）。
