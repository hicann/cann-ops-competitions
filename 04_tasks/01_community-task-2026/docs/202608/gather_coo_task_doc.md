...
- 分档为 **达标基线**；**验收取最优实现**；
- Gather 为 **访存密集型**（读 src + 写 out），950PR 128B Sector 对离散复制友好。

| 场景分档 | 特征 | dtype | **达标基线（≥ A100）** |
|----------|------|-------|------------------------|
| S1 边特征广播 | index 长 = 边数 | float32 | **0.60×** |
| S2 高维 feature | K 较大 | float32 | **0.65×** |
| S3 float16 | 常规 GNN | float16 | **0.70×** |
| S4 L2 路径 | L2 | float64/int64 | 不做性能考核 |

**验收用例**：

| 编号 | 场景 | src | index 长 | 达标 |
|------|------|-----|----------|------|
| P-01 | 图边 gather | `[N, F]` | `nnz` | ≥ 0.60× |
| P-02 | 广播 batch | `[B, N, F]` | `[B, nnz]` | ≥ 0.60× |
| P-03 | 与 gather_csr 同数据 | 对齐 indptr | - | 耗时相当 |
