# 8月社区任务 - MatMul 算子开发任务书

## 1. 任务概述

### 1.1 开发目标
基于 **Ascend C C API** 接口开发纯 Cube-Core 矩阵乘法算子 MatMul（Matrix Multiplication）。算子逻辑为两个输入矩阵完成矩阵相乘运算，叠加偏置向量后输出结果矩阵。
* **核心约束**：

  *   请使用 Cube-Core 单元完成`MatMul`计算。
  *   算子采用**单核**方案开发，无需处理分块（Tiling）相关逻辑。
...
*   **数学公式**：
    ...
    > 说明：A、B、bias表示三个输入矩阵（其中bias是偏置矩阵），形状分别是 [i, k] 、 [k, j] 和 [1, j]。
...
### 1.3 交付流程
完成代码编写、单元测试、性能调优后，提交 **PR** 到仓库 `master` 分支的 `examples/02_simd_c_api/03_c_api/03_matrix_compute` 目录下（PR 提交规范见参考资料（https://gitcode.com/cann/asc-devkit/wiki/05_%E7%A0%94%E5%8F%91%E5%8D%8F%E4%BD%9C%E8%A7%84%E8%8C%83.md ）），并附带完整的测试报告。


## 2. 核心开发要求

### 2.1 功能实现
1. **对齐标准**：功能逻辑需与 Ascend C 官方算子库（https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/latest/API/aolapi/atlasoxol_09_0092.html ）中的 `matmul` 实现完全对齐（注意需要带偏置矩阵）。
...
计划基于官方自验证样例（https://gitcode.com/cann/asc-devkit/tree/master/examples/02_simd_c_api/03_c_api/03_matrix_compute/mmad ）完成修改：
1. 将`mmad.asc`中当前占位调用：
    ```cpp
...
开发者可以参考现有样例（https://gitcode.com/cann/asc-devkit/blob/master/examples/02_simd_c_api/03_c_api/03_matrix_compute/mmad/scripts/gen_data.py ）中的验证方案并自行修改。
3. 精度要求
    - 数据大小范围：[-100,  100]
