...
参考昇腾版本内置aclnnEqual算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子,并且实现方式从原来比较二进制的实现方式，更改成和cpu一致的比较逻辑值的实现方式，完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。
...
3. 代码样例：[https://gitcode.com/cann/ops-math/tree/master/math/tensor_equal](https://gitcode.com/cann/ops-math/tree/master/math/tensor_equal)
