...
参考昇腾版本内置aclnnPdist算子的 TBE 实现，在昇腾 NPU 上基于 Ascend C 编程语言实现功能一致的算子，并且修复p=inf场景存在的精度问题，完成算子设计、开发、测试全流程工作，验收通过后将算子提交至昇腾算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 与原 TBE 算子核心功能完全对齐，支持原算子对应的所有数据类型、数据格式，并且修复p=inf场景存在的精度问题。
...
3. 代码样例：[https://gitcode.com/cann/ops-math/tree/master/math/pdist](https://gitcode.com/cann/ops-math/tree/master/math/pdist)
