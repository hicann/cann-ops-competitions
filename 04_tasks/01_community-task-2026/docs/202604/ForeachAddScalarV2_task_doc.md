# aclnnForeachAddScalarV2算子开发任务书
...
当前aclnnForeachAddScalarV2算子不支持DT_INT16/DT_INT8/DT_UINT8数据类型，需要在原来的代码上进行再开发，使其支持DT_INT16/DT_INT8/DT_UINT8数据类型。要求完成算子设计，算子开发及算子测试任务。验收后需将该算子提交到算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 请通过修改 <https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_add_scalar_list> 目录下的文件，使其支持DT_INT16/DT_INT8/DT_UINT8数据。注意还应修改相应文档，补充DT_INT16/DT_INT8/DT_UINT8的支持情况。
...
验收通过后，在昇腾算子开源仓提交 PR 申请，申请将开发完成的算子合入 [https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_add_scalar_list](https://gitcode.com/cann/ops-nn/tree/master/foreach)。
