...
当前aclnnMaxUnpool2d算子的self输入不支持bf16数据类型，需要在原来的代码上进行再开发，使其支持bf16数据类型，并且indices支持int64/int32。要求完成算子设计，算子开发及算子测试任务。验收后需将该算子提交到算子开源仓。

## 核心开发要求及验收标准

### 功能实现要求

1. 使self输入支持bf16数据，indices支持int32/int64。注意还应修改相应文档，补充的支持情况。
...
> 性能和fp16数据类型持平。Indices int32与int64性能持平
