...
extern "C" unsigned int HcclAICPUKernel(OpParam *param)
{
    ...
    if (param->resCtx == nullptr || param->ctxSize != sizeof(AlgResourceCtx)) {
        HCCL_ERROR("Invalid AllGather resource context, addr=%p, size=%llu, expected=%zu",
    ...
    if (HcommThreadNotifyWaitOnThread(
            resCtx->aicpuThread, ALG_HOST_TO_AICPU_NOTIFY_IDX, CUSTOM_TIMEOUT) != HCCL_SUCCESS) {
