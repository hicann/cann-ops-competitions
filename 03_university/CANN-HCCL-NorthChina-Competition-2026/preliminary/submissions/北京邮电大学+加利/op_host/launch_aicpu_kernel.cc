...
thread_local aclrtFuncHandle g_kernelFuncHandle = nullptr;

// 加载 AICPU Kernel
HcclResult LoadAICPUKernel()
{
    // 避免重复加载
    if (g_binKernelHandle != nullptr && g_kernelFuncHandle != nullptr) {
        return HCCL_SUCCESS;
    }
    ...
    ACLCHECK(aclrtBinaryGetFunction(
        g_binKernelHandle, "HcclAICPUKernel", &g_kernelFuncHandle));
    return HCCL_SUCCESS;
}
} // namespace
...
HcclResult LaunchAICPUKernel(OpParam &param, aclrtStream stream)
{
    ...
    CHK_RET(HcommThreadNotifyRecordOnThread(
        param.cpuThread, param.aicpuThreadOnCpu, ALG_HOST_TO_AICPU_NOTIFY_IDX));

    aclrtArgsHandle argsHandle;

    // 构造 Kernel 函数入参
    ACLCHECK(aclrtKernelArgsInit(g_kernelFuncHandle, &argsHandle));
    ...
    ACLCHECK(aclrtLaunchKernelWithConfig(
        g_kernelFuncHandle, numBlocks, stream, &cfg, argsHandle, nullptr));
