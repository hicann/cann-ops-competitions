...
HcclResult LaunchAICPUKernel(OpParam &param, aclrtStream stream)
{
    ...
    // AllGather 不使用 root 参数；复用该字段携带 AICPU 主 thread 的 Host 同步 notify 索引。
    CHK_RET(HcommThreadNotifyRecordOnThread(param.cpuThread, param.aicpuThreadOnCpu, param.root));
