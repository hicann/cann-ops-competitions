...
HcclResult AcquireFullMeshChannels(HcclComm comm, const OpParam &param, AlgResourceCtx &resCtxHost)
{
    CHK_PRT_RET(param.rankSize < 2 || (param.rankSize % 2) != 0,
        HCCL_ERROR("[AcquireFullMeshChannels] Invalid rankSize[%u].", param.rankSize), HCCL_E_PARA);
    ...
    for (uint32_t remoteRank = 0; remoteRank < param.rankSize; ++remoteRank) {
        ...
        if (!isSameServer) {
            // 对称映射使每个 rank 的跨机 peer 均匀使用 Clos 物理链路。
            preferredLinkIndex = (param.myRank % localRankSize + remoteRank % localRankSize) % localRankSize;
        }
    ...
    void *buf, uint64_t count, HcclDataType dataType, uint32_t root, HcclComm comm, aclrtStream stream)
{
    ...
    // 构造算子参数
    OpParam param;
    sprintf(param.tag, "%s", "hccl_custom_broadcast_hierarchical_pipeline_v1");
    param.inputPtr = buf;
    param.outputPtr = buf;
    param.count = count;
    ...
    if (HcclEngineCtxGet(comm, param.tag, aicpuTsEngine, &ctx, &size) == HCCL_SUCCESS) {
        ...
        uint32_t notifyNumPerThread = std::max(1U, threadNum - 1);
        ...
        if (param.rankSize > 1) {
            CHK_RET(AcquireFullMeshChannels(comm, param, resCtxHost));
        }
