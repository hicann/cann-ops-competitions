...
struct ChannelInfo {
    ...
    uint32_t layer = 0; ///< 该channel走的网络层: 0=Server内Mesh(layer0), 1=Server间Clos(layer1)
};
...
struct AlgResourceCtx {
    ...
    std::vector<uint32_t> sameServerRanks;  ///< 本Server的rank列表(含自己, layer0)
    std::vector<uint32_t> otherServerRanks; ///< 对端Server的rank列表(layer1可达)
    ...
    std::vector<char> Serialize()
    {
        ...
        binaryStream << sameServerRanks;
        binaryStream << otherServerRanks;
