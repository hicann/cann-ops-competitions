...
CcuResult CcuReduceScatterWriteReduce4x1Kernel(CcuKernelArg arg)
{
    ...
    localAccumulator.addr = accumulator;
    localAccumulator.token = accumulatorToken;
    ccu::Event completed;
    ...
    // The address publication overlaps the self copy.  The token is published
    // only after initialization completes, so observing both XN bits proves
    // that endpoint WriteReduce may safely update the peer accumulator.
    for (uint32_t channel = 0; channel < kernelArg->channelCount; ++channel) {
        CCU_RETURN_IF_ERROR(ccu::WriteVariableWithNotify(
            kernelArg->channels[channel], accumulator, INPUT_XN_ID,
            CKE_IDX_0, 1U << INPUT_XN_ID));
    }
    CCU_RETURN_IF_ERROR(ccu::EventWait(completed, SELF_COPY_BIT));
    for (uint32_t channel = 0; channel < kernelArg->channelCount; ++channel) {
        CCU_RETURN_IF_ERROR(ccu::WriteVariableWithNotify(
            kernelArg->channels[channel], accumulatorToken, TOKEN_XN_ID,
    ...
    for (uint32_t round = 0; round < WRITE_REDUCE_4X1_STRIPES;
        ...
        for (uint32_t channel = 0; channel < kernelArg->channelCount;
            ...
            const uint32_t stripe
                ...
                % WRITE_REDUCE_4X1_STRIPES;
            remoteDestination[channel].addr = peerAccumulator[channel];
        ...
        // Use one distinct bit per wave so a fast rank can never record a bit
        // twice before a slow peer consumes it.  INPUT/TOKEN were globally
        // consumed by READY_BITS before wave 0, so each is safe to reuse once.
        const uint32_t barrierBit = round == 0U
            ? (1U << POST_SYNC_ID)
            : (round == 1U ? (1U << INPUT_XN_ID)
                           : (1U << TOKEN_XN_ID));
        for (uint32_t channel = 0; channel < kernelArg->channelCount;
            ++channel) {
    ...
    // Close the final TOKEN-bit lifecycle before the next collective can
    // publish its pre-sync token.  POST is safe to reuse here because reaching
    // waves 1 and 2 proves that every peer consumed wave 0's POST bit.  Seeing
    // a peer's final ACK in turn proves that peer consumed this rank's wave 2
    // TOKEN bit.
    for (uint32_t channel = 0; channel < kernelArg->channelCount; ++channel) {
        CCU_RETURN_IF_ERROR(ccu::NotifyRecord(
    ...
    finalOutput.addr = output;
    finalOutput.token = outputToken;
    CCU_RETURN_IF_ERROR(ccu::LocalCopy(finalOutput, localAccumulator,
        outputLength, completed, 1U));
    CCU_RETURN_IF_ERROR(ccu::EventWait(completed, 1U));
    return CCU_SUCCESS;
}
