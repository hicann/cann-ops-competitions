...
constexpr uint64_t RANK16_512M_RECV_COUNT = 8388608ULL;
constexpr uint64_t RANK16_TAIL_RECV_COUNT = 6553600ULL;
constexpr uint64_t RANK4_512M_RECV_COUNT = 33554432ULL;
constexpr uint64_t RANK4_TAIL_RECV_COUNT = 26214400ULL;

bool SelectV6WideLane(const OpParam &param)
{
    return param.rankSize == 16U
        && (param.count == RANK16_512M_RECV_COUNT
            || param.count == RANK16_TAIL_RECV_COUNT);
}

bool SelectV2FourLane(const OpParam &param)
{
    return param.rankSize == 4U
        && (param.count == RANK4_512M_RECV_COUNT
            || param.count == RANK4_TAIL_RECV_COUNT);
}
...
bool V6WideLaneFits(uint64_t elementCount, uint64_t localBufferSize)
{
    if (elementCount > std::numeric_limits<uint64_t>::max() / sizeof(float)) {
        return false;
    }
    const uint64_t outputBytes = elementCount * sizeof(float);
    std::array<uint64_t, DIRECT_TILE_COUNT> tileLengths{};
    uint64_t workspaceStride = 0;
    if (!BuildDirectWideTiles(outputBytes, tileLengths, workspaceStride)
        || outputBytes > localBufferSize) {
        return false;
    }
    constexpr uint32_t WIDE_SLOT_COUNT = 2U * WIDE_LANE_WORKSPACE_BANKS
        * (MAX_GROUP_SOURCE_COUNT - 1U);
    return workspaceStride
        <= (localBufferSize - outputBytes) / WIDE_SLOT_COUNT;
}
...
{
    // The two measured rank-16 large cases intentionally retain V6's direct
    // two-IO-die wide-lane graph instead of entering the V11 hybrid graph.
    if (SelectV6WideLane(param)
        && V6WideLaneFits(param.count, localBufferSize)) {
        return false;
    }
    ...
    Hetero12WorkspacePlan heteroPlan;
    return CalculateHetero12Workspace(param.count, heteroPlan)
        && std::max(heteroPlan.h8Required, heteroPlan.h4Required)
            <= localBufferSize;
}
...
HcclResult RegisterHetero12Kernels(HcclComm comm, const OpParam &param,
    ...
    const HierarchyRankPlan *hierarchyPlan, const MateNodeLayout *mateLayout, AlgResourceCtx &resource)
{
    ...
    if (mateLayout != nullptr) {
        if (param.rankSize == 12) {
            return RegisterHetero12Kernels(
    ...
    const bool v2FourLane = SelectV2FourLane(param)
        && groups.size() == 1U
        && outputBytes
            <= resource.localBuffer.size / (MAX_ASYNC_STRIPES - 1U);
    const bool writeReduce4x1 = !v2FourLane
        && param.rankSize == 4U && groups.size() == 1U
    ...
    for (size_t groupIndex = 0; groupIndex < groups.size(); ++groupIndex) {
        ...
        CHK_PRT_RET(v2FourLane && sourceCount != 4U,
            HCCL_ERROR("[RegisterKernels] V2 four-lane path needs four sources"),
            HCCL_E_INTERNAL);
        if (v2FourLane) {
            kernelArg->stripeCount = 0;
        }
    ...
    HcclComm comm, aclrtStream stream)
{
    ...
    const int tagLen = std::snprintf(param.tag, sizeof(param.tag),
        "hccl_custom_reducescatter_yesoryes_v6_v2_v12e_%u_%llu",
        param.rankSize, static_cast<unsigned long long>(recvCount));
