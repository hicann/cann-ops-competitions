...
namespace {
    ...
    bool DeserializeRuntimeContext(const void *data, uint64_t size, RuntimeResourceCtx &ctx)
    {
        ...
        return ReadContextValue(cursor, end, compatibilityThread)
            ...
            && ReadContextValue(cursor, end, ctx.cachedOutputToken) && cursor == end;
    }
...
HcclResult ExecOp(const OpParam &param)
{
    ...
    const bool stagedSchedule
        = hybridSchedule || hetero12Schedule != Hetero12Schedule::NONE;
    ...
    if (singleWriteReduce4x1) {
        CHK_PRT_RET(outputBytes > resCtx.localBuffer.size,
            HCCL_ERROR(
                "[ExecOp] 4x1 WriteReduce accumulator needs %llu bytes, have %llu",
                static_cast<unsigned long long>(outputBytes),
                static_cast<unsigned long long>(resCtx.localBuffer.size)),
            HCCL_E_INTERNAL);
        const std::array<uint64_t, 6> writeReduceArgs = {
            inputAddress,
            outputAddress,
            scratchAddress,
            inputToken,
