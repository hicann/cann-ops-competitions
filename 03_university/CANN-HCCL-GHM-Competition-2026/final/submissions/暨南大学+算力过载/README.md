...
├── op_kernel_ccu/                  # CCU侧代码目录（决赛构建入口）
│   ├── ccu_kernel.h                # ★ 选手编写：CCU Kernel声明
│   └── ccu_kernel.cc               # ★ 选手编写：CCU Kernel实现
└── op_kernel_aicpu/                # 初赛 AICPU 代码（按锁定分支完整保留，不参与决赛构建）
```

> [!NOTE] 注意：
> 本目录归档的是决赛 CCU 版本。决赛关键文件为 `custom.h`、`reduce_scatter.cc`、
> `op_host/exec_op.h`、`op_host/exec_op.cc`、`ccu_kernel.h`、`ccu_kernel.cc`。
> 构建集成沿用官方仓库中已收录 HCCL 决赛 CCU 工程的标准目录关系。
