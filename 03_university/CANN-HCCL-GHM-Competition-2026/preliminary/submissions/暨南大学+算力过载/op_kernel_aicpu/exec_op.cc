...
constexpr uint64_t PARALLEL_MAX_OUTPUT_BYTES = 32ULL * 1024 * 1024;
constexpr uint32_t PARALLEL_RANK_SIZE = 16;
constexpr uint32_t PARALLEL_THREAD_NUM = 2;
constexpr uint32_t FIRST_WORKER_MASK = 9;
...
HcclResult ExecParallelLayer1(const OpParam &param, const AlgResourceCtx &resCtx,
    const std::vector<const ChannelInfo *> &channelByRank, uint64_t totalBytes,
    HcommDataType hcommDataType, HcommReduceOp hcommReduceOp)
{
    CHK_PRT_RET(resCtx.threads.size() < PARALLEL_THREAD_NUM,
        HCCL_ERROR("ExecOp: parallel layer-1 path requires[%u] threads, got[%zu]",
            PARALLEL_THREAD_NUM, resCtx.threads.size()),
        HCCL_E_INTERNAL);
    CHK_PRT_RET(totalBytes > std::numeric_limits<uint64_t>::max() / PARALLEL_THREAD_NUM,
        HCCL_ERROR("ExecOp: parallel workspace byte size overflows"), HCCL_E_INTERNAL);
    const uint64_t workspaceBytes = totalBytes * PARALLEL_THREAD_NUM;
    CHK_PRT_RET(workspaceBytes > resCtx.localBuffer.size,
        HCCL_ERROR("ExecOp: local HCCL buffer[%llu] is smaller than parallel workspace[%llu]",
            static_cast<unsigned long long>(resCtx.localBuffer.size),
            static_cast<unsigned long long>(workspaceBytes)),
        HCCL_E_INTERNAL);
    for (const ChannelInfo &channel : resCtx.channels) {
        CHK_PRT_RET(channel.remoteCclMem.addr == nullptr || channel.remoteCclMem.size < workspaceBytes,
            HCCL_ERROR("ExecOp: remote HCCL buffer of rank[%u] cannot hold workspace[%llu]",
                channel.remoteRank, static_cast<unsigned long long>(workspaceBytes)),
            HCCL_E_INTERNAL);
    }

    ThreadHandle localThread = resCtx.threads[0];
    ThreadHandle crossThread = resCtx.threads[1];
    auto *input = static_cast<uint8_t *>(param.inputPtr);
    auto *output = static_cast<uint8_t *>(param.outputPtr);
    auto *localPartial = static_cast<uint8_t *>(resCtx.localBuffer.addr);
    auto *crossPartial = localPartial + totalBytes;
    const uint64_t selfBlockOffset = static_cast<uint64_t>(param.myRank) * totalBytes;

    // The worker cannot start before the host-stream wait queued on thread 0.
    CHK_RET(static_cast<HcclResult>(
        HcommThreadNotifyRecordOnThread(localThread, crossThread, 0)));
    CHK_RET(static_cast<HcclResult>(
        HcommThreadNotifyWaitOnThread(crossThread, 0, CUSTOM_TIMEOUT)));

    // The main thread starts first, so give it masks 1..8. The worker starts
    // after a local notify and handles masks 9..15. This 8/7 network split
    // compensates for worker startup latency without adding tasks or buffers.
    CHK_RET(CopyBytesOnThread(
        localThread, localPartial, input + selfBlockOffset, totalBytes));
    for (uint32_t mask = 1; mask < FIRST_WORKER_MASK; ++mask) {
        const uint32_t peer = param.myRank ^ mask;
        const ChannelInfo &channel = *channelByRank[peer];
        const uint64_t sendBlockOffset = static_cast<uint64_t>(peer) * totalBytes;
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
            localThread, channel.handle, NOTIFY_IDX_ACK)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
            localThread, channel.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
        CHK_RET(static_cast<HcclResult>(HcommWriteReduceOnThread(localThread, channel.handle,
            channel.remoteCclMem.addr, input + sendBlockOffset, param.count,
            hcommDataType, hcommReduceOp)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
            localThread, channel.handle, NOTIFY_IDX_DATA_SIGNAL)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
            localThread, channel.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
    }

    // Thread 1 independently accumulates seven contributions. Mask 9 initializes
    // slot 1 with a plain write; masks 10..15 reduce into it.
    const uint32_t firstCrossPeer = param.myRank ^ FIRST_WORKER_MASK;
    const ChannelInfo &firstCrossChannel = *channelByRank[firstCrossPeer];
    const uint64_t firstCrossSendOffset = static_cast<uint64_t>(firstCrossPeer) * totalBytes;
    auto *firstCrossRemoteSlot =
        static_cast<uint8_t *>(firstCrossChannel.remoteCclMem.addr) + totalBytes;
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
        crossThread, firstCrossChannel.handle, NOTIFY_IDX_ACK)));
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
        crossThread, firstCrossChannel.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
    CHK_RET(static_cast<HcclResult>(HcommWriteOnThread(crossThread, firstCrossChannel.handle,
        firstCrossRemoteSlot, input + firstCrossSendOffset, totalBytes)));
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
        crossThread, firstCrossChannel.handle, NOTIFY_IDX_DATA_SIGNAL)));
    CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
        crossThread, firstCrossChannel.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));

    for (uint32_t mask = FIRST_WORKER_MASK + 1; mask < param.rankSize; ++mask) {
        const uint32_t peer = param.myRank ^ mask;
        const ChannelInfo &channel = *channelByRank[peer];
        const uint64_t sendBlockOffset = static_cast<uint64_t>(peer) * totalBytes;
        auto *remoteSlot = static_cast<uint8_t *>(channel.remoteCclMem.addr) + totalBytes;
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
            crossThread, channel.handle, NOTIFY_IDX_ACK)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
            crossThread, channel.handle, NOTIFY_IDX_ACK, CUSTOM_TIMEOUT)));
        CHK_RET(static_cast<HcclResult>(HcommWriteReduceOnThread(crossThread, channel.handle,
            remoteSlot, input + sendBlockOffset, param.count, hcommDataType, hcommReduceOp)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyRecordOnThread(
            crossThread, channel.handle, NOTIFY_IDX_DATA_SIGNAL)));
        CHK_RET(static_cast<HcclResult>(HcommChannelNotifyWaitOnThread(
            crossThread, channel.handle, NOTIFY_IDX_DATA_SIGNAL, CUSTOM_TIMEOUT)));
    }

    // Copy the worker partial to the user output while the main lane may still
    // be reducing masks 1..8. The join below guarantees that this copy is
    // complete before the main thread performs the final in-place reduction.
    CHK_RET(CopyBytesOnThread(crossThread, output, crossPartial, totalBytes));

    // Join both independent communication streams. Since output already holds
    // the worker partial, reduce the main partial directly into output and avoid
    // a separate full-size copy after the join.
    CHK_RET(static_cast<HcclResult>(
        HcommThreadNotifyRecordOnThread(crossThread, localThread, 0)));
    CHK_RET(static_cast<HcclResult>(
        HcommThreadNotifyWaitOnThread(localThread, 0, CUSTOM_TIMEOUT)));
    CHK_RET(static_cast<HcclResult>(HcommLocalReduceOnThread(localThread,
        output, localPartial, param.count, hcommDataType, hcommReduceOp)));
    return HCCL_SUCCESS;
}
...
HcclResult ExecOp(const OpParam &param, const AlgResourceCtx &resCtx)
{
    ...
    if (param.rankSize == PARALLEL_RANK_SIZE && totalBytes <= PARALLEL_MAX_OUTPUT_BYTES) {
        HCCL_INFO("ExecOp: use dual-lane layer-1 path, bytes[%llu]",
            static_cast<unsigned long long>(totalBytes));
        return ExecParallelLayer1(
            param, resCtx, channelByRank, totalBytes, hcommDataType, hcommReduceOp);
    }
