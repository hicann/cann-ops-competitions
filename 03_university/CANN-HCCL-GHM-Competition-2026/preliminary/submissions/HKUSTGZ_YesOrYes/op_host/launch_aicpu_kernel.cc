...
thread_local aclrtFuncHandle g_funcKernelHandle = nullptr;
...
HcclResult LaunchAICPUKernel(OpParam &param, aclrtStream stream)
{
    ...
    // 获取 Kernel 函数句柄
    if (g_funcKernelHandle == nullptr) {
        constexpr char kernelName[] = "HcclAICPUKernel";
        ACLCHECK(aclrtBinaryGetFunction(g_binKernelHandle, kernelName, &g_funcKernelHandle));
    }

    // 每次下发均重新构造参数，避免跨迭代复用已 finalize 的 ArgsHandle。
    aclrtArgsHandle argsHandle;
    ACLCHECK(aclrtKernelArgsInit(g_funcKernelHandle, &argsHandle));
    ...
    ACLCHECK(aclrtLaunchKernelWithConfig(g_funcKernelHandle, numBlocks, stream, &cfg, argsHandle, nullptr));
