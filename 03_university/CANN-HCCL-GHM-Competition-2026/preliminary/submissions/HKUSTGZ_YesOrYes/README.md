...
本实现按数据量选择不同 ReduceScatter 路径。512KB 数据采用 Recursive Halving，并使用 `READ_REDUCE` 减少双向同步次数；512MB 数据采用 Rolling Stripe，将输出块切分为多条 stripe，并通过逐 stripe 流水让下一块写入与当前块归约重叠；400MB+4B 数据采用双 bank Tree Pipeline，在接收、归约和释放 buffer 之间形成流水。Host 侧根据数据规模申请对应的线程、channel、notify 和 CCL buffer，并通过引擎上下文缓存复用资源。
