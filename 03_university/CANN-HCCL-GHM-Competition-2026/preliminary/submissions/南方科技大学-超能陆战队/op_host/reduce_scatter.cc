...
HcclResult GetNetLayers(HcclComm comm, std::vector<uint32_t> &netLayers)
{
    ...
    CHK_PRT_RET(layerList == nullptr || layerCount == 0, HCCL_ERROR("No network layer found in the communicator"),
        HCCL_E_INTERNAL);

    // RankGraph 返回的内存由 HCCL 管理，后续查询可能使其失效，因此立即复制。
    netLayers.assign(layerList, layerList + layerCount);
    return HCCL_SUCCESS;
}
    ...
    uint32_t remoteRank, HcclChannelDesc &desc)
{
    for (uint32_t netLayer : netLayers) {
        ...
        CHK_RET(HcclRankGraphGetLinks(comm, netLayer, localRank, remoteRank, &linkList, &linkCount));
        if (linkCount == 0) {
            continue;
        }

        // 同一对端只能申请一个 Channel；复制首条实际链路后立即构造描述符。
        ...
        HCCL_INFO("Use topology layer %u for channel from rank %u to rank %u", netLayer, localRank, remoteRank);
        return HCCL_SUCCESS;
    }

    HCCL_ERROR("No link from rank %u to rank %u in any topology layer", localRank, remoteRank);
    return HCCL_E_INTERNAL;
}

HcclResult AcquireChannels(HcclComm comm, CommEngine engine, const OpParam &param, AlgResourceCtx &resCtx)
{
    if (param.rankSize <= 1) {
        return HCCL_SUCCESS;
    }
    ...
    std::vector<uint32_t> remoteRanks(channelCount);
    ...
    for (uint32_t remoteRank = 0; remoteRank < param.rankSize; ++remoteRank) {
        ...
        CHK_RET(FillChannelDesc(comm, netLayers, param.myRank, remoteRank, descs[channelIndex]));
        remoteRanks[channelIndex] = remoteRank;
        ++channelIndex;
    }

    CHK_RET(HcclChannelAcquire(comm, engine, descs.data(), channelCount, handles.data()));

    resCtx.channels.reserve(channelCount);
    ...
    HcclComm comm, aclrtStream stream)
{
    ...
    if (HcclEngineCtxGet(comm, param.tag, aicpuTsEngine, &ctx, &size) == HCCL_SUCCESS) {
        ...
        // 每个对端使用一条独立 Thread/Channel，使 15 个远端分片可以并行读取。
        // 主 Thread 还要接收其余 Thread 的完成通知，因此为每条 Thread 申请 threadNum 个 Notify。
        ...
        CHK_RET(AcquireChannels(comm, aicpuTsEngine, param, resCtxHost));
