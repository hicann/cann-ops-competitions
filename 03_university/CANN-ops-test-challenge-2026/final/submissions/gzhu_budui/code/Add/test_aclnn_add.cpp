                                 ...
                                 aclnnStatus (*runFn)(void*, uint64_t, aclOpExecutor*, aclrtStream))
{
    *wsAddr = nullptr;
    if (wsSize > 0) {
        ...
        CHECK_RET(ret == ACL_SUCCESS, return static_cast<aclnnStatus>(ret));
    }
...
{
    return static_cast<int32_t>(static_cast<uint32_t>(a) + static_cast<uint32_t>(alpha) * static_cast<uint32_t>(b));
}
...
static void TestAddFP32Alpha1(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnAdd); 
        aclrtSynchronizeStream(stream); 
    }
    
    std::vector<float> res(8, 0);
    if (ok) {
        aclrtMemcpy(res.data(), 8 * sizeof(float), dout, 8 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 8 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i]), 1.0), 1e-6, 1e-6);
    }
    ...
    LOG_PRINT("Test case %s\n  Expected: [1,3,5,7,9,11,13,15]\n  Actual: [%.1f,%.1f,%.1f,%.1f,...]\n",
              name, static_cast<double>(res[0]), static_cast<double>(res[1]), static_cast<double>(res[2]), static_cast<double>(res[3]));
    ReportTest(name, ok);
}
...
static void TestAddFP32AlphaAxpy(aclrtStream stream)
{
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(float), dout, 4 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i]), 1.5), 1e-6, 1e-6);
    }
...
static void TestAddFP32AlphaZero(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i]), 0.0), 1e-6, 1e-6);
    }
...
static void TestAddFP32AlphaNeg(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i]), -2.0), 1e-6, 1e-6);
    }
...
static void TestAddFP16Alpha1(aclrtStream stream)
{
    ...
    std::vector<uint16_t> x1(4);
    std::vector<uint16_t> x2(4);
    std::vector<uint16_t> outData(4, 0);
    
    for (int i = 0; i < 4; ++i) { 
        x1[i] = FloatToFP16(x1f[i]);
        x2[i] = FloatToFP16(x2f[i]); 
    }
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(uint16_t), dout, 4 * sizeof(uint16_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(FP16ToFloat(x2[i])), 1.0);
        ok &= CheckClose(static_cast<double>(FP16ToFloat(res[i])), expected, 1e-3, 1e-3);
    }
...
static void TestAddFP16AlphaAxpy(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(FP16ToFloat(x2[i])), 2.0);
        ok &= CheckClose(static_cast<double>(FP16ToFloat(res[i])), expected, 1e-2, 1e-2);
    }
...
static void TestAddBF16Alpha1(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4; ++i) { 
        x1[i] = FloatToBF16(x1f[i]);
        x2[i] = FloatToBF16(x2f[i]); 
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(BF16ToFloat(x1[i])), static_cast<double>(BF16ToFloat(x2[i])), 1.0);
        ok &= CheckClose(static_cast<double>(BF16ToFloat(res[i])), expected, 1e-2, 1e-2);
    }
...
static void TestAddBF16AlphaAxpy(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(BF16ToFloat(x1[i])), static_cast<double>(BF16ToFloat(x2[i])), 2.0);
        ok &= CheckClose(static_cast<double>(BF16ToFloat(res[i])), expected, 1e-1, 1e-2);
    }
...
static void TestAddINT32Alpha1(aclrtStream stream)
{
    ...
    int32_t av = 1; 
    aclScalar* alpha = aclCreateScalar(&av, ACL_INT32);
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(int32_t), dout, 4 * sizeof(int32_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
...
static void TestAddINT8Alpha1(aclrtStream stream)
{
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(int8_t), dout, 4 * sizeof(int8_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<int8_t>(static_cast<uint8_t>(x1[i]) + static_cast<uint8_t>(x2[i])));
    }
...
static void TestAddUINT8Alpha1(aclrtStream stream)
{
    ...
    uint8_t av = 1; 
    aclScalar* alpha = aclCreateScalar(&av, ACL_UINT8);
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(uint8_t), dout, 4 * sizeof(uint8_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<uint8_t>(x1[i] + x2[i]));
    }
...
static void TestAddINT64Alpha1(aclrtStream stream)
{
    ...
    int64_t av = 1LL; 
    aclScalar* alpha = aclCreateScalar(&av, ACL_INT64);
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(int64_t), dout, 4 * sizeof(int64_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
...
static void TestAddBOOLAlpha1(aclrtStream stream)
{
    ...
    uint8_t av = 1; 
    aclScalar* alpha = aclCreateScalar(&av, ACL_BOOL);
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<uint8_t>(static_cast<bool>(x1[i]) | static_cast<bool>(x2[i])));
    }
...
static void TestAddMixFP16FP32(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(x2[i]), 1.0);
        ok &= CheckClose(static_cast<double>(res[i]), expected, 1e-3, 1e-3);
    }
...
static void TestAddMixFP32FP16(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double expected = CpuAddF(static_cast<double>(x1[i]), static_cast<double>(FP16ToFloat(x2[i])), 1.0);
        ok &= CheckClose(static_cast<double>(res[i]), expected, 1e-3, 1e-3);
    }
...
static void TestAddBroadcast(aclrtStream stream)
{
    ...
    std::vector<int64_t> sh1 = {4, 1};
    std::vector<int64_t> sh2 = {4, 4};
    std::vector<int64_t> shout = {4, 4};
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 16 * sizeof(float), dout, 16 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        for (int j = 0; j < 4 && ok; ++j) {
            ok &= CheckClose(static_cast<double>(res[i * 4 + j]),
                             CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i * 4 + j]), 1.0), 1e-6, 1e-6);
        }
    }
...
static void TestAddLargeTensor(aclrtStream stream)
{
    ...
    for (int i = 0; i < N; ++i) { 
        x1[i] = static_cast<float>(i % 100);
        x2[i] = static_cast<float>(i % 50 + 1); 
    }
    ...
    if (ok) {
        aclrtMemcpy(res.data(), N * sizeof(float), dout, N * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < N && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(x2[i]), 1.0), 1e-5, 1e-5);
    }
...
static void TestAdds(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnAdds); 
        aclrtSynchronizeStream(stream); 
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(scalarVal), 1.0), 1e-6, 1e-6);
    }
...
static void TestAddsAlpha(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(x1[i]), static_cast<double>(scalarVal), 2.5), 1e-5, 1e-5);
    }
...
static void TestInplaceAdd(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnInplaceAdd); 
        aclrtSynchronizeStream(stream);
    }
    
    std::vector<float> res(4, 0);
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(float), d1, 4 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
...
static void TestInplaceAdds(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnInplaceAdds); 
        aclrtSynchronizeStream(stream); 
    }
...
static void TestAddV3Alpha1(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnAddV3); 
        aclrtSynchronizeStream(stream); 
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(selfVal), static_cast<double>(x2[i]), 1.0), 1e-6, 1e-6);
    }
...
static void TestAddV3Axpy(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(selfVal), static_cast<double>(x2[i]), 2.0), 1e-6, 1e-6);
    }
...
static void TestAddV3MulAdd(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        int8_t expected = static_cast<int8_t>(static_cast<uint8_t>(selfVal) + static_cast<uint8_t>(2) * static_cast<uint8_t>(x2[i]));
        ok &= (res[i] == expected);
    }
...
static void TestInplaceAddV3(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wsAddr, ws, exec, stream, aclnnInplaceAddV3); 
        aclrtSynchronizeStream(stream); 
    }
    
    std::vector<float> res(4, 0);
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(float), d2, 4 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
...
static void TestPrecisionLargeSmall(aclrtStream stream)
{
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 2 * sizeof(float), dout, 2 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    // Expected (double precision): 10000000000.00001
    // FP32 cannot represent: float ULP at 1e10 ≈ 1024, far larger than 1e-5
    double expected = CpuAddF(static_cast<double>(x1[0]), static_cast<double>(x2[0]), 1.0);
    double error = std::abs(static_cast<double>(res[0]) - expected);
    bool hasPrecisionLoss = (error > 1e-4 && ok);
    
    LOG_PRINT("Test case %s\n  Expected (double): %.9f\n  Actual (FP32):     %.9f\n"
              "  Error: %.9f\n  %s\n", name, expected, static_cast<double>(res[0]), error,
...
static void TestPrecisionCancellation(aclrtStream stream)
{
    ...
    double exp0 = CpuAddF(static_cast<double>(x1[0]), static_cast<double>(x2[0]), 1.0);
    double exp1 = CpuAddF(static_cast<double>(x1[1]), static_cast<double>(x2[1]), 1.0);
    double err0 = std::abs(static_cast<double>(res[0]) - exp0);
    
    LOG_PRINT("Test case %s\n  x1=1.0000001, x2=-1.0\n"
              "  Expected (CPU double): %.10e\n  Actual (FP32):         %.10e\n  Error: %.3e\n",
              name, exp0, static_cast<double>(res[0]), err0);
              
    // Operator is correct to FP32 precision; the visible "error" vs math truth is a FP32 property
    bool verified = ok && CheckClose(static_cast<double>(res[0]), exp0, 1e-6f, 1e-6f) && 
                    CheckClose(static_cast<double>(res[1]), exp1, 1e-6f, 1e-6f);
...
static void TestPrecisionAlphaError(aclrtStream stream)
{
    ...
    double expected = CpuAddF(1.0, 3.0, static_cast<double>(av));
    double mathTrue = 1.0 + 3.0 / 3.0;  // = 2.0
    
    LOG_PRINT("Test case %s\n  Math truth: %.10f  CPU ref (float alpha): %.10f  NPU: %.10f\n"
              "  Error vs math truth: %.3e\n",
              name, mathTrue, expected, static_cast<double>(res[0]), std::abs(static_cast<double>(res[0]) - mathTrue));
              
    bool verified = ok && CheckClose(static_cast<double>(res[0]), expected, 1e-6, 1e-6);
...
static void TestPrecisionInf(aclrtStream stream)
{
    ...
    LOG_PRINT("Test case %s\n  Expected: +inf\n  Actual: %f  isInf=%d\n",
              name, static_cast<double>(res[0]), static_cast<int>(isInf));
...
static void TestPrecisionNaN(aclrtStream stream)
{
    ...
    bool normalOk = CheckClose(static_cast<double>(res[1]), 3.0, 1e-6, 1e-6);
    
    LOG_PRINT("Test case %s\n  res[0](NaN+1.0)=%f isNaN=%d  res[1](1.0+2.0)=%f\n",
              name, static_cast<double>(res[0]), static_cast<int>(nanPropagated), static_cast<double>(res[1]));
...
static void TestPrecisionDtypeCompare(aclrtStream stream)
{
    ...
    void* d2_32 = nullptr;
    void* dout32 = nullptr;
    aclTensor* t1_32 = nullptr;
    aclTensor* t2_32 = nullptr;
    aclTensor* tout32 = nullptr;
    
    float av32 = 1.0f; 
    aclScalar* alpha32 = aclCreateScalar(&av32, ACL_FLOAT);
    ...
    uint64_t ws32 = 0;
    aclOpExecutor* exec32 = nullptr; 
    void* wsAddr32 = nullptr;
    
    bool ok32 = (aclnnAddGetWorkspaceSize(t1_32, t2_32, alpha32, tout32, &ws32, &exec32) == ACL_SUCCESS);
    if (ok32) { 
        ExecWorkspace(&wsAddr32, ws32, exec32, stream, aclnnAdd); 
        aclrtSynchronizeStream(stream); 
    }
    
    std::vector<float> res32(4, 0);
    if (ok32) {
        aclrtMemcpy(res32.data(), 4 * sizeof(float), dout32, 4 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    }

    // FP16 test
    std::vector<uint16_t> x1_16(4);
    std::vector<uint16_t> x2_16(4);
    std::vector<uint16_t> out16(4, 0);
    for (int i = 0; i < 4; ++i) { 
        x1_16[i] = FloatToFP16(x1f[i]); 
        x2_16[i] = FloatToFP16(x2f[i]);
    }
    
    void* d1_16 = nullptr;
    void* d2_16 = nullptr;
    void* dout16 = nullptr;
    aclTensor* t1_16 = nullptr;
    aclTensor* t2_16 = nullptr;
    aclTensor* tout16 = nullptr;
    
    float av16 = 1.0f; 
    aclScalar* alpha16 = aclCreateScalar(&av16, ACL_FLOAT);
    ...
    uint64_t ws16 = 0;
    aclOpExecutor* exec16 = nullptr; 
    void* wsAddr16 = nullptr;
    
    bool ok16 = (aclnnAddGetWorkspaceSize(t1_16, t2_16, alpha16, tout16, &ws16, &exec16) == ACL_SUCCESS);
    if (ok16) { 
        ExecWorkspace(&wsAddr16, ws16, exec16, stream, aclnnAdd); 
        aclrtSynchronizeStream(stream); 
    }
    
    std::vector<uint16_t> res16(4, 0);
    if (ok16) {
        aclrtMemcpy(res16.data(), 4 * sizeof(uint16_t), dout16, 4 * sizeof(uint16_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    LOG_PRINT("Test case %s\n  FP32: [%.6f,%.6f,%.6f,%.6f]\n  FP16: [%.4f,%.4f,%.4f,%.4f]\n",
              name, static_cast<double>(res32[0]), static_cast<double>(res32[1]), static_cast<double>(res32[2]), static_cast<double>(res32[3]),
              static_cast<double>(FP16ToFloat(res16[0])), static_cast<double>(FP16ToFloat(res16[1])),
              static_cast<double>(FP16ToFloat(res16[2])), static_cast<double>(FP16ToFloat(res16[3])));
              
    bool fp32ok = ok32;
    bool fp16ok = ok16;
    
    for (int i = 0; i < 4 && ok32; ++i) {
        fp32ok &= CheckClose(static_cast<double>(res32[i]), CpuAddF(static_cast<double>(x1f[i]), static_cast<double>(x2f[i]), 1.0), 1e-5, 1e-5);
    }
        
    for (int i = 0; i < 4 && ok16; ++i) {
        double exp16 = CpuAddF(static_cast<double>(FP16ToFloat(x1_16[i])), static_cast<double>(FP16ToFloat(x2_16[i])), 1.0);
        fp16ok &= CheckClose(static_cast<double>(FP16ToFloat(res16[i])), exp16, 1e-2, 1e-2);
    }
    
    aclDestroyTensor(t1_32); 
    aclDestroyTensor(t2_32); 
    aclDestroyScalar(alpha32); 
    aclDestroyTensor(tout32);
    
    aclDestroyTensor(t1_16); 
    aclDestroyTensor(t2_16); 
    aclDestroyScalar(alpha16); 
    aclDestroyTensor(tout16);
                   ...
                   aclTensor* tout, aclrtStream stream, bool* ok)
{
    ...
    *ok = (aclnnAddGetWorkspaceSize(t1, t2, al, tout, &ws, &exec) == ACL_SUCCESS);
    if (*ok) {
        *ok = (ExecWorkspace(&wa, ws, exec, stream, aclnnAdd) == ACL_SUCCESS);
    }
    if (*ok) {
...
static void RunAdds(aclTensor* t1, aclScalar* other, aclScalar* alpha,
                    aclTensor* tout, aclrtStream stream, bool* ok)
{
    uint64_t ws = 0;
    aclOpExecutor* exec = nullptr;
    ...
    *ok = (aclnnAddsGetWorkspaceSize(t1, other, alpha, tout, &ws, &exec) == ACL_SUCCESS);
    if (*ok) {
        *ok = (ExecWorkspace(&wa, ws, exec, stream, aclnnAdds) == ACL_SUCCESS);
    }
    if (*ok) {
        *ok = (aclrtSynchronizeStream(stream) == ACL_SUCCESS);
    }
...
static void RunInplaceAdd(aclTensor* t1, aclTensor* t2, aclScalar* alpha,
                          aclrtStream stream, bool* ok)
{
    uint64_t ws = 0;
    aclOpExecutor* exec = nullptr;
    void* wa = nullptr;

    *ok = (aclnnInplaceAddGetWorkspaceSize(t1, t2, alpha, &ws, &exec) == ACL_SUCCESS);
    if (*ok) {
        *ok = (ExecWorkspace(&wa, ws, exec, stream, aclnnInplaceAdd) == ACL_SUCCESS);
    }
    if (*ok) {
        *ok = (aclrtSynchronizeStream(stream) == ACL_SUCCESS);
    }
...
static void RunAddV3(aclScalar* self, aclTensor* other, aclScalar* alpha,
                     aclTensor* out, aclrtStream stream, bool* ok)
{
    uint64_t ws = 0;
    aclOpExecutor* exec = nullptr;
    void* wa = nullptr;

    *ok = (aclnnAddV3GetWorkspaceSize(self, other, alpha, out, &ws, &exec) == ACL_SUCCESS);
    if (*ok) {
        *ok = (ExecWorkspace(&wa, ws, exec, stream, aclnnAddV3) == ACL_SUCCESS);
    }
    if (*ok) {
        *ok = (aclrtSynchronizeStream(stream) == ACL_SUCCESS);
    }
...
static void RunInplaceAddV3(aclScalar* self, aclTensor* other, aclScalar* alpha,
                            aclrtStream stream, bool* ok)
{
    uint64_t ws = 0;
    aclOpExecutor* exec = nullptr;
    ...
    *ok = (aclnnInplaceAddV3GetWorkspaceSize(self, other, alpha, &ws, &exec) == ACL_SUCCESS);
    if (*ok) {
        *ok = (ExecWorkspace(&wa, ws, exec, stream, aclnnInplaceAddV3) == ACL_SUCCESS);
    }
    if (*ok) {
        *ok = (aclrtSynchronizeStream(stream) == ACL_SUCCESS);
    }
...
static void TestAddDOUBLEAlpha1(aclrtStream stream)
{
    ...
    std::vector<double> x2 = {5.0, 6.0, 7.0, 8.0};
    ...
    double av = 1.0; 
    aclScalar* alpha = aclCreateScalar(&av, ACL_DOUBLE);
    ...
    bool ok; 
    RunAdd(t1, t2, alpha, tout, stream, &ok);
    
    std::vector<double> res(4, 0.0);
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(double), dout, 4 * sizeof(double), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(res[i], x1[i] + x2[i], 1e-10, 1e-10);
    }
...
static void TestAddINT16Alpha1(aclrtStream stream)
{
    ...
    std::vector<int64_t> sh = {4};
    std::vector<int16_t> x1 = {10, 20, 30, 40};
    std::vector<int16_t> x2 = {1, 2, 3, 4};
    ...
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(int16_t), dout, 4 * sizeof(int16_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<int16_t>(x1[i] + x2[i]));
    }
...
static void TestAddINT8AlphaAxpyV2(aclrtStream stream)
{
    ...
    std::vector<int64_t> sh = {4};
    std::vector<int8_t> x1 = {1, 2, 3, 4};
    std::vector<int8_t> x2 = {5, 6, 7, 8};
    std::vector<int8_t> out(4, 0);
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<int8_t>(static_cast<uint8_t>(x1[i]) + static_cast<uint8_t>(av) * static_cast<uint8_t>(x2[i])));
    }
...
static void TestAddUINT8AlphaAxpyV2(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<uint8_t>(x1[i] + av * x2[i]));
    }
...
static void TestAddINT64AlphaAxpyV2(aclrtStream stream)
{
    ...
    std::vector<int64_t> sh = {4};
    std::vector<int64_t> x1 = {10, 20, 30, 40};
    std::vector<int64_t> x2 = {1, 2, 3, 4};
    std::vector<int64_t> out(4, 0);
...
static void TestAddMixFP16FP32AlphaNe1(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(x2[i]), 2.0), 1e-2, 1e-2);
    }
...
{
    const char* name = "TC-43 aclnnAdds FP16 + FP16 scalar alpha=1 (stable promote path)";
    std::vector<int64_t> sh = {4};
    std::vector<uint16_t> x1 = {FloatToFP16(1.0f), FloatToFP16(2.0f), FloatToFP16(3.0f), FloatToFP16(4.0f)};
    ...
    uint16_t scv = FloatToFP16(1.0f);
    uint16_t alv = FloatToFP16(1.0f);
    aclScalar* sc = aclCreateScalar(&scv, ACL_FLOAT16);
    aclScalar* al = aclCreateScalar(&alv, ACL_FLOAT16);
    
    CreateAclTensor(x1, sh, &d1, ACL_FLOAT16, &t1);
    CreateAclTensor(out, sh, &dout, ACL_FLOAT, &tout);

    bool ok = false;
    RunAdds(t1, sc, al, tout, stream, &ok);
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]),
                         CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(FP16ToFloat(scv)), 1.0), 5e-2, 5e-2);
    }
...
static void TestAddsIsKeepB16False(aclrtStream stream)
{
    ...
    float alv = 1.0f;
    aclScalar* sc = aclCreateScalar(&scv, ACL_FLOAT);
    aclScalar* al = aclCreateScalar(&alv, ACL_FLOAT);
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(scv), 1.0), 2e-3, 2e-3);
    }
...
static void TestAddsBOOLToINT32(aclrtStream stream)
{
    ...
    bool scv = true;
    bool alv = true;
    aclScalar* sc = aclCreateScalar(&scv, ACL_BOOL);
    aclScalar* al = aclCreateScalar(&alv, ACL_BOOL);
    ...
    if (ok) { 
        ExecWorkspace(&wa, ws, exec, stream, aclnnAdds); 
        aclrtSynchronizeStream(stream); 
    }
...
static void TestAddsINT32Axpy(aclrtStream stream)
{
    ...
    int32_t scv = 5;
    int32_t alv = 3;
    aclScalar* sc = aclCreateScalar(&scv, ACL_INT32);
    aclScalar* al = aclCreateScalar(&alv, ACL_INT32);
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == CpuAddI32(x1[i], scv, alv));
    }
...
// TC-48: Adds INT64 alpha=1 → stable INT64 path in Adds
static void TestAddsINT64AxpyV2(aclrtStream stream)
{
    const char* name = "TC-48 aclnnAdds INT64 alpha=1 (stable INT64 path)";
    std::vector<int64_t> sh = {4};
    std::vector<int64_t> x1 = {100, 200, 300, 400};
    std::vector<int64_t> out(4, 0);
    ...
    int64_t scv = 10LL;
    int64_t alv = 1LL;
    aclScalar* sc = aclCreateScalar(&scv, ACL_INT64);
    aclScalar* al = aclCreateScalar(&alv, ACL_INT64);
...
static void TestInplaceAddFP16(aclrtStream stream)
{
    ...
    bool ok = false;
    RunInplaceAdd(t1, t2, alpha, stream, &ok);
    ...
    if (!ok) {
        ...
        aclrtFree(d1);
        aclrtFree(d2);
        LOG_PRINT("  [SKIP] %s (runtime does not stably support FP16 inplace path)\n", name);
        return;
    }
    
    std::vector<uint16_t> res(4, 0);
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(uint16_t), d1, 4 * sizeof(uint16_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        double exp = CpuAddF(static_cast<double>(FP16ToFloat(x1[i])), static_cast<double>(FP16ToFloat(x2[i])), 1.0);
        ok &= CheckClose(static_cast<double>(FP16ToFloat(res[i])), exp, 1e-2, 1e-2);
    }
...
static void TestInplaceAddBF16(aclrtStream stream)
{
    ...
    if (!ok) {
        ...
        aclrtFree(d1);
        aclrtFree(d2);
        LOG_PRINT("  [SKIP] %s (runtime does not stably support BF16 inplace path)\n", name);
        return;
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double exp = CpuAddF(static_cast<double>(BF16ToFloat(x1[i])), static_cast<double>(BF16ToFloat(x2[i])), 1.0);
        ok &= CheckClose(static_cast<double>(BF16ToFloat(res[i])), exp, 1e-1, 1e-2);
    }
...
static void TestInplaceAddsINT32Alpha2(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wa, ws, exec, stream, aclnnInplaceAdds); 
        aclrtSynchronizeStream(stream);
    }
    
    std::vector<int32_t> res(4, 0);
    if (ok) {
        aclrtMemcpy(res.data(), 4 * sizeof(int32_t), d1, 4 * sizeof(int32_t), ACL_MEMCPY_DEVICE_TO_HOST);
    }
        
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= (res[i] == static_cast<int32_t>(static_cast<uint32_t>(x1[i]) + 2 * static_cast<uint32_t>(scv)));
    }
...
static void TestAddV3FP16Axpy(aclrtStream stream)
{
    ...
    aclScalar* al = aclCreateScalar(&av, ACL_FLOAT);
    ...
    if (ok) { 
        ExecWorkspace(&wa, ws, exec, stream, aclnnAddV3); 
        aclrtSynchronizeStream(stream); 
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double exp = CpuAddF(static_cast<double>(sv), static_cast<double>(FP16ToFloat(x2[i])), 2.0);
        ok &= CheckClose(static_cast<double>(FP16ToFloat(res[i])), exp, 5e-2, 5e-2);
    }
...
static void TestAddV3INT32Axpy(aclrtStream stream)
{
    ...
    aclScalar* al = aclCreateScalar(&av, ACL_INT32);
    
    CreateAclTensor(x2, sh, &d2, ACL_INT32, &t2);
    CreateAclTensor(out, sh, &dout, ACL_INT32, &tout);
    
    bool ok = false;
    RunAddV3(sc, t2, al, tout, stream, &ok);
...
static void TestAddV3BF16MulAdd(aclrtStream stream)
{
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        double exp = CpuAddF(static_cast<double>(sv), static_cast<double>(BF16ToFloat(x2[i])), 2.0);
        ok &= CheckClose(static_cast<double>(BF16ToFloat(res[i])), exp, 1e-1, 1e-1);
    }
...
static void TestInplaceAddV3Axpy(aclrtStream stream)
{
    ...
    if (ok) { 
        ExecWorkspace(&wa, ws, exec, stream, aclnnInplaceAddV3); 
        aclrtSynchronizeStream(stream);
    }
    ...
    for (int i = 0; i < 4 && ok; ++i) {
        ok &= CheckClose(static_cast<double>(res[i]), CpuAddF(static_cast<double>(sv), static_cast<double>(x2[i]), 2.0), 1e-5, 1e-5);
    }
...
// ===================================================================
// SECTION 12: Extended high-value coverage & precision cases (TC-92~TC-111)
// ===================================================================
static void TestExtendedCoveragePrecision(aclrtStream stream)
{
    struct AddFp32Case {
        const char* name;
        std::vector<float> x1;
    ...
    const std::vector<AddFp32Case> addFp32Cases = {
        {"TC-92 Add FP32 alpha=1e-4 tiny-scale", {1.0f, -2.0f, 3.0f, -4.0f}, {1e-3f, -1e-3f, 2e-3f, -2e-3f}, 1e-4f, 1e-6, 1e-6},
        {"TC-93 Add FP32 alpha=1e5 large-finite", {1.0f, 2.0f, 3.0f, 4.0f}, {1.0f, -1.0f, 2.0f, -2.0f}, 1e5f, 1e-3, 1e-6},
        {"TC-94 Add FP32 alpha=-0.75 fractional-neg", {10.5f, -20.25f, 30.125f, -40.5f}, {2.0f, -3.0f, 4.0f, -5.0f}, -0.75f, 1e-5, 1e-5},
        {"TC-95 Add FP32 alpha=-0.0 signed-zero", {7.0f, -8.0f, 9.0f, -10.0f}, {123.0f, 456.0f, -789.0f, -321.0f}, -0.0f, 1e-6, 1e-6},
        {"TC-96 Add FP32 alpha=1e-37 subnormal-edge", {1.0f, 2.0f, -3.0f, -4.0f}, {1e20f, -1e20f, 1e20f, -1e20f}, 1e-37f, 1e-6, 1e-6},
        {"TC-97 Add FP32 cancellation mixed-sign", {10000.125f, -10000.125f, 1.0001f, -1.0001f}, {-10000.0f, 10000.0f, -1.0f, 1.0f}, 1.0f, 1e-6, 1e-6}
    };

    for (const auto& tc : addFp32Cases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x1.size())};
        std::vector<float> out(tc.x1.size(), 0.0f);
        std::vector<float> res(tc.x1.size(), 0.0f);
        ...
        float alphaVal = tc.alpha;
        aclScalar* alpha = aclCreateScalar(&alphaVal, ACL_FLOAT);
        CreateAclTensor(tc.x1, sh, &d1, ACL_FLOAT, &t1);
        CreateAclTensor(tc.x2, sh, &d2, ACL_FLOAT, &t2);
        CreateAclTensor(out, sh, &dout, ACL_FLOAT, &tout);

        bool ok = false;
        RunAdd(t1, t2, alpha, tout, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(float), dout, res.size() * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
        }

        for (size_t i = 0; i < res.size() && ok; ++i) {
            const double expected = CpuAddF(static_cast<double>(tc.x1[i]), static_cast<double>(tc.x2[i]), static_cast<double>(tc.alpha));
            ok &= CheckClose(static_cast<double>(res[i]), expected, tc.atol, tc.rtol);
        }
    ...
    const std::vector<AddI32Case> addI32Cases = {
        {"TC-98 Add INT32 near-overflow alpha=2", {std::numeric_limits<int32_t>::max() - 3, std::numeric_limits<int32_t>::max() - 100, 123456, -123456}, {2, 50, -1000, 2000}, 2},
        {"TC-99 Add INT32 mixed-sign alpha=-3", {100, -200, 300, -400}, {7, -8, 9, -10}, -3}
    };

    for (const auto& tc : addI32Cases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x1.size())};
        std::vector<int32_t> out(tc.x1.size(), 0);
        std::vector<int32_t> res(tc.x1.size(), 0);
        ...
        int32_t alphaVal = tc.alpha;
        aclScalar* alpha = aclCreateScalar(&alphaVal, ACL_INT32);
        CreateAclTensor(tc.x1, sh, &d1, ACL_INT32, &t1);
        CreateAclTensor(tc.x2, sh, &d2, ACL_INT32, &t2);
        CreateAclTensor(out, sh, &dout, ACL_INT32, &tout);

        bool ok = false;
        RunAdd(t1, t2, alpha, tout, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(int32_t), dout, res.size() * sizeof(int32_t), ACL_MEMCPY_DEVICE_TO_HOST);
        }

        for (size_t i = 0; i < res.size() && ok; ++i) {
            ok &= (res[i] == CpuAddI32(tc.x1[i], tc.x2[i], tc.alpha));
        }
    ...
    const std::vector<AddsFp32Case> addsFp32Cases = {
        {"TC-100 Adds FP32 scalar=1e-3 alpha=1e-4", {1.0f, 2.0f, 3.0f, 4.0f}, 1e-3f, 1e-4f},
        {"TC-101 Adds FP32 scalar=3.5 alpha=-2", {-1.0f, 2.0f, -3.0f, 4.0f}, 3.5f, -2.0f},
        {"TC-102 Adds FP32 scalar=2 alpha=1e4", {1.0f, -1.0f, 10.0f, -10.0f}, 2.0f, 1e4f},
        {"TC-103 Adds FP32 scalar=7 alpha=0", {5.0f, -6.0f, 7.0f, -8.0f}, 7.0f, 0.0f}
    };

    for (const auto& tc : addsFp32Cases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x.size())};
        std::vector<float> out(tc.x.size(), 0.0f);
        std::vector<float> res(tc.x.size(), 0.0f);
        ...
        float scalarVal = tc.scalar;
        float alphaVal = tc.alpha;
        aclScalar* other = aclCreateScalar(&scalarVal, ACL_FLOAT);
        aclScalar* alpha = aclCreateScalar(&alphaVal, ACL_FLOAT);
        CreateAclTensor(tc.x, sh, &d1, ACL_FLOAT, &t1);
        CreateAclTensor(out, sh, &dout, ACL_FLOAT, &tout);

        bool ok = false;
        RunAdds(t1, other, alpha, tout, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(float), dout, res.size() * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
        }

        for (size_t i = 0; i < res.size() && ok; ++i) {
            const double expected = CpuAddF(static_cast<double>(tc.x[i]), static_cast<double>(tc.scalar), static_cast<double>(tc.alpha));
            ok &= CheckClose(static_cast<double>(res[i]), expected, 1e-5, 1e-5);
        }
    ...
    const std::vector<InplaceAddCase> inplaceAddCases = {
        {"TC-104 InplaceAdd FP32 alpha=0 identity", {1.0f, -2.0f, 3.0f, -4.0f}, {9.0f, 9.0f, 9.0f, 9.0f}, 0.0f},
        {"TC-105 InplaceAdd FP32 alpha=-1 subtraction", {10.0f, -20.0f, 30.0f, -40.0f}, {1.0f, 2.0f, 3.0f, 4.0f}, -1.0f},
        {"TC-106 InplaceAdd FP32 alpha=0.125 fractional", {8.0f, 16.0f, -24.0f, -32.0f}, {8.0f, -8.0f, 8.0f, -8.0f}, 0.125f}
    };

    for (const auto& tc : inplaceAddCases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x1.size())};
        std::vector<float> res(tc.x1.size(), 0.0f);
        ...
        RunInplaceAdd(t1, t2, alpha, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(float), d1, res.size() * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
        }

        for (size_t i = 0; i < res.size() && ok; ++i) {
            const double expected = CpuAddF(static_cast<double>(tc.x1[i]), static_cast<double>(tc.x2[i]), static_cast<double>(tc.alpha));
            ok &= CheckClose(static_cast<double>(res[i]), expected, 1e-6, 1e-6);
        }
    ...
    const std::vector<AddV3Case> addV3Cases = {
        {"TC-107 AddV3 FP32 alpha=0 self-only", 5.0f, {1.0f, 2.0f, 3.0f, 4.0f}, 0.0f},
        {"TC-108 AddV3 FP32 alpha=-2.5 negative-scale", -3.0f, {1.0f, -2.0f, 3.0f, -4.0f}, -2.5f},
        {"TC-109 AddV3 FP32 alpha=1e-4 tiny-scale", 1e4f, {1.0f, -1.0f, 2.0f, -2.0f}, 1e-4f}
    };

    for (const auto& tc : addV3Cases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x2.size())};
        std::vector<float> out(tc.x2.size(), 0.0f);
        std::vector<float> res(tc.x2.size(), 0.0f);
        ...
        bool ok = false;
        RunAddV3(self, t2, alpha, tout, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(float), dout, res.size() * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
        }

        for (size_t i = 0; i < res.size() && ok; ++i) {
            const double expected = CpuAddF(static_cast<double>(tc.self), static_cast<double>(tc.x2[i]), static_cast<double>(tc.alpha));
            ok &= CheckClose(static_cast<double>(res[i]), expected, 1e-5, 1e-5);
        }
    ...
    const std::vector<InplaceAddV3Case> inplaceAddV3Cases = {
        {"TC-110 InplaceAddV3 FP32 alpha=-1.5", 6.0f, {2.0f, -2.0f, 4.0f, -4.0f}, -1.5f},
        {"TC-111 InplaceAddV3 FP32 alpha=0", 8.0f, {1.0f, 3.0f, 5.0f, 7.0f}, 0.0f}
    };

    for (const auto& tc : inplaceAddV3Cases) {
        std::vector<int64_t> sh = {static_cast<int64_t>(tc.x2.size())};
        std::vector<float> res(tc.x2.size(), 0.0f);
        ...
        bool ok = false;
        RunInplaceAddV3(self, t2, alpha, stream, &ok);
        if (ok) {
            aclrtMemcpy(res.data(), res.size() * sizeof(float), d2, res.size() * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
        }
...
{
    std::vector<int64_t> sh4 = {4};
    std::vector<int64_t> sh3 = {3};
    std::vector<float> fv4(4, 1.0f);
    std::vector<float> fv3(3, 1.0f);
    ...
    void* dBig9 = nullptr;
    void* dBool = nullptr;
    void* dBoolOut = nullptr;
    void* dI32Out = nullptr;
    
    aclTensor* tA = nullptr;
    aclTensor* tB = nullptr;
    aclTensor* tOut = nullptr;
    aclTensor* tSmall = nullptr;
    aclTensor* tU = nullptr;
    aclTensor* tBig9 = nullptr;
    aclTensor* tBool = nullptr;
    aclTensor* tBoolOut = nullptr;
    aclTensor* tI32Out = nullptr;
    ...
    std::vector<int64_t> sh9(9, 1LL);
    std::vector<int64_t> st9(9, 1LL);
    ...
    ReportTest("TC-57 Err_Add_null_self", NE(aclnnAddGetWorkspaceSize(nullptr, tB, alF, tOut, &ws, &e)));
    ReportTest("TC-58 Err_Add_null_other", NE(aclnnAddGetWorkspaceSize(tA, nullptr, alF, tOut, &ws, &e)));
    ReportTest("TC-59 Err_Add_null_alpha", NE(aclnnAddGetWorkspaceSize(tA, tB, nullptr, tOut, &ws, &e)));
    ReportTest("TC-60 Err_Add_null_out", NE(aclnnAddGetWorkspaceSize(tA, tB, alF, nullptr, &ws, &e)));
    ReportTest("TC-61 Err_Add_unsupported_UINT32", NE(aclnnAddGetWorkspaceSize(tU, tU, alF, tU, &ws, &e)));
    ReportTest("TC-62 Err_Add_dim_exceed_8", NE(aclnnAddGetWorkspaceSize(tBig9, tBig9, alF, tBig9, &ws, &e)));
    ReportTest("TC-63 Err_Add_promote_outcast_BOOL", NE(aclnnAddGetWorkspaceSize(tA, tB, alF, tBoolOut, &ws, &e)));
    ReportTest("TC-64 Err_Add_BOOL_FLOAT_alpha", NE(aclnnAddGetWorkspaceSize(tBool, tBool, alF, tBoolOut, &ws, &e)));
    ReportTest("TC-65 Err_Add_shape_out_mismatch", NE(aclnnAddGetWorkspaceSize(tA, tB, alF, tSmall, &ws, &e)));
    
    // --- aclnnAdds null / invalid checks ---
    ReportTest("TC-66 Err_Adds_null_self", NE(aclnnAddsGetWorkspaceSize(nullptr, osF, alF, tOut, &ws, &e)));
    ReportTest("TC-67 Err_Adds_null_other", NE(aclnnAddsGetWorkspaceSize(tA, nullptr, alF, tOut, &ws, &e)));
    ReportTest("TC-68 Err_Adds_null_alpha", NE(aclnnAddsGetWorkspaceSize(tA, osF, nullptr, tOut, &ws, &e)));
    ReportTest("TC-69 Err_Adds_null_out", NE(aclnnAddsGetWorkspaceSize(tA, osF, alF, nullptr, &ws, &e)));
    ReportTest("TC-70 Err_Adds_unsupported_UINT32", NE(aclnnAddsGetWorkspaceSize(tU, osF, alF, tU, &ws, &e)));
    ReportTest("TC-71 Err_Adds_shape_mismatch", NE(aclnnAddsGetWorkspaceSize(tA, osF, alF, tSmall, &ws, &e)));
    ReportTest("TC-72 Err_Adds_dim_exceed_8", NE(aclnnAddsGetWorkspaceSize(tBig9, osF, alF, tBig9, &ws, &e)));
    ReportTest("TC-73 Err_Adds_promote_outcast_BOOL", NE(aclnnAddsGetWorkspaceSize(tA, osF, alF, tBoolOut, &ws, &e)));
    
    // --- aclnnInplaceAdd null / shape checks ---
    ReportTest("TC-74 Err_InplaceAdd_null_self", NE(aclnnInplaceAddGetWorkspaceSize(nullptr, tB, alF, &ws, &e)));
    ReportTest("TC-75 Err_InplaceAdd_null_other", NE(aclnnInplaceAddGetWorkspaceSize(tA, nullptr, alF, &ws, &e)));
    ReportTest("TC-76 Err_InplaceAdd_null_alpha", NE(aclnnInplaceAddGetWorkspaceSize(tA, tB, nullptr, &ws, &e)));
    ReportTest("TC-77 Err_InplaceAdd_broadcast_ne_self", NE(aclnnInplaceAddGetWorkspaceSize(tSmall, tB, alF, &ws, &e)));
    
    // --- aclnnInplaceAdds null checks ---
    ReportTest("TC-78 Err_InplaceAdds_null_self", NE(aclnnInplaceAddsGetWorkspaceSize(nullptr, osF, alF, &ws, &e)));
    ReportTest("TC-79 Err_InplaceAdds_null_other", NE(aclnnInplaceAddsGetWorkspaceSize(tA, nullptr, alF, &ws, &e)));
    ReportTest("TC-80 Err_InplaceAdds_null_alpha", NE(aclnnInplaceAddsGetWorkspaceSize(tA, osF, nullptr, &ws, &e)));
    
    // --- aclnnAddV3 null & invalid checks ---
    ReportTest("TC-81 Err_AddV3_null_self", NE(aclnnAddV3GetWorkspaceSize(nullptr, tB, alF, tOut, &ws, &e)));
    ReportTest("TC-82 Err_AddV3_null_other", NE(aclnnAddV3GetWorkspaceSize(osF, nullptr, alF, tOut, &ws, &e)));
    ReportTest("TC-83 Err_AddV3_null_alpha", NE(aclnnAddV3GetWorkspaceSize(osF, tB, nullptr, tOut, &ws, &e)));
    ReportTest("TC-84 Err_AddV3_null_out", NE(aclnnAddV3GetWorkspaceSize(osF, tB, alF, nullptr, &ws, &e)));
    ReportTest("TC-85 Err_AddV3_unsupported_UINT32", NE(aclnnAddV3GetWorkspaceSize(osF, tU, alF, tU, &ws, &e)));
    ReportTest("TC-86 Err_AddV3_dim_exceed_8", NE(aclnnAddV3GetWorkspaceSize(osF, tBig9, alF, tBig9, &ws, &e)));
    ReportTest("TC-87 Err_AddV3_promote_outcast_BOOL", NE(aclnnAddV3GetWorkspaceSize(osF, tB, alF, tBoolOut, &ws, &e)));
    ReportTest("TC-88 Err_AddV3_shape_mismatch", NE(aclnnAddV3GetWorkspaceSize(osF, tB, alF, tSmall, &ws, &e)));
    
    // --- aclnnInplaceAddV3 null checks ---
    ReportTest("TC-89 Err_InplaceAddV3_null_self", NE(aclnnInplaceAddV3GetWorkspaceSize(nullptr, tB, alF, &ws, &e)));
    ReportTest("TC-90 Err_InplaceAddV3_null_other", NE(aclnnInplaceAddV3GetWorkspaceSize(osF, nullptr, alF, &ws, &e)));
    ReportTest("TC-91 Err_InplaceAddV3_null_alpha", NE(aclnnInplaceAddV3GetWorkspaceSize(osF, tB, nullptr, &ws, &e)));

    aclDestroyTensor(tA); 
    aclDestroyTensor(tB); 
    aclDestroyTensor(tOut);
    aclDestroyTensor(tSmall); 
    aclDestroyTensor(tU); 
    aclDestroyTensor(tBig9);
    aclDestroyTensor(tBool); 
    aclDestroyTensor(tBoolOut); 
    aclDestroyTensor(tI32Out);
    
    aclDestroyScalar(alF); 
    aclDestroyScalar(osF); 
    aclDestroyScalar(alBl); 
    aclDestroyScalar(osBl);
...
int main()
{
    ...
    // Section 12: Extended high-value coverage and precision cases
    LOG_PRINT("\n--- Section 12: Extended coverage/precision (TC-92~TC-111) ---\n");
    TestExtendedCoveragePrecision(stream);
