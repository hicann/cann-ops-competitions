...
/// Ring AllReduce 算法配置参数
struct AlgConfig {
    uint32_t prevRank = INVALID_VALUE_RANKID; ///< 环形拓扑中的前驱 Rank
    uint32_t nextRank = INVALID_VALUE_RANKID; ///< 环形拓扑中的后继 Rank
};
...
struct AlgResourceCtx {
    ...
    AlgConfig algConfig;               ///< 通信算法配置（Ring Topology）
    ...
    std::vector<char> Serialize()
    {
        ...
        binaryStream << algConfig.prevRank;
        binaryStream << algConfig.nextRank;
